...
Ext.define("Ext.XTemplateParser", {
	...
	doTpl : Ext.emptyFn,
	parse : function(n) {
		var o = this, l = n.length, c = {
			elseif : "elif"
		}, j = o.topRe, b = o.actionsRe, k, p, r, f, q, g, a, e, d, h, i;
		o.level = 0;
		o.stack = p = [];
		for (k = 0; k < l; k = h) {
			j.lastIndex = k;
...
Ext.define("Ext.app.Action", {
	...
	resume : function() {
		var b = this.getCurrentFilterIndex() + 1, c = this.getBeforeFilters(), a = this
				.getController(), d = c[b];
		if (d) {
...
Ext.define("Ext.app.Route", {
			...
			initialize : function() {
				...
				this.paramsInMatchString = this.getUrl()
						.match(this.paramMatchingRegex)
						|| [];
			...
			argsFor : function(c) {
				var b = [], f = this.paramsInMatchString, a = c
						.match(this.matcherRegex), e = f.length, d;
				a.shift();
				for (d = 0; d < e; d++) {
					b.push(f[d].replace(":", ""));
...
Ext.define("Ext.data.JsonP", {
	...
	request : function(m) {
		m = Ext.apply({}, m);
		var i = this, d = Ext.isDefined(m.disableCaching)
				? m.disableCaching
				: i.disableCaching, g = m.disableCachingParam
				|| i.disableCachingParam, c = ++i.statics().requestCount, k = m.callbackName
				|| "callback" + c, h = m.callbackKey || i.callbackKey, l = Ext
				.isDefined(m.timeout) ? m.timeout : i.timeout, e = Ext.apply(
				{}, m.params), b = m.url, a = Ext.isSandboxed ? Ext
				.getUniqueGlobalNamespace() : "Ext", f, j;
		e[h] = a + ".data.JsonP." + k;
		if (d) {
...
Ext.define("Ext.data.Operation", {
	...
	applyModel : function(a) {
		if (typeof a == "string") {
			...
			if (!a) {
				Ext.Logger.error("Model with name " + arguments[0]
						+ " doesnt exist.")
			}
		}
		if (a && !a.prototype.isModel && Ext.isObject(a)) {
			a = Ext.data.ModelManager.registerType(a.storeId || a.id
							|| Ext.id(), a)
		}
		return a
	},
						...
						DATE : {
							convert : function(e) {
								...
								if (isNaN(d)) {
									...
									if (isNaN(d)) {
										d = new Date(Date.parse(e.replace(
												this.dashesRe, "/")))
									}
								}
...
Ext.define("Ext.data.identifier.Uuid", {
			...
			generate : function() {
				var c = this, e = c.parts, a = c.getVersion(), b = c.getSalt(), d = c
						.getTimestamp();
				e[0] = c.toHex(d.lo, 8);
				e[1] = c.toHex(d.hi & 65535, 4);
				e[2] = c.toHex(((d.hi >>> 16) & 4095) | (a << 12), 4);
				e[3] = c.toHex(128 | ((c.clockSeq >>> 8) & 63), 2)
						+ c.toHex(c.clockSeq & 255, 2);
...
Ext.define("Ext.data.writer.Writer", {
	...
	getRecordData : function(e) {
		var j = e.phantom === true, b = this.getWriteAllFields() || j, c = this
				.getNameProperty(), f = e.getFields(), d = {}, h, a, g, k, i;
		if (b) {
			f.each(function(l) {
						if (l.getPersist()) {
...
Ext.define("Ext.data.writer.Xml", {
	...
	writeRecords : function(a, b) {
		var g = this, d = [], c = 0, f = b.length, h = g.getDocumentRoot(), e = g
				.getRecord(), l = b.length !== 1, k, j;
		d.push(g.getHeader() || "");
		if (!h && l) {
...
Ext.define("Ext.event.Controller", {
	...
	doFire : function() {
		var k = this.firingListeners, c = this.firingArguments, g = c.length
				- 2, d, f, b, o, h, n, a, j, l, e, m;
...
Ext.define("Ext.event.recognizer.Recognizer", {
			...
			fire : function() {
				this.getOnRecognized()
						.apply(this.getCallbackScope(), arguments)
			}
		});
...
Ext.define("Ext.fx.easing.Bounce", {
			...
			getValue : function() {
				var b = Ext.Date.now() - this.getStartTime(), c = (b / this
						.getAcceleration()), a = c
						* Math.pow(Math.E, -this.getSpringTension() * c);
...
Ext.define("Ext.fx.easing.Linear", {
			...
			getValue : function() {
				var a = Ext.Date.now() - this.getStartTime(), b = this
						.getDuration();
				if (a > b) {
					this.isEnded = true;
...
Ext.define("Ext.fx.easing.Momentum", {
			...
			getValue : function() {
				return this.getStartValue() - this.velocity
...
Ext.define("Ext.mixin.Selectable", {
	...
	selectAll : function(a) {
		var e = this, c = e.getStore().getRange(), d = c.length, b = 0;
		for (; b < d; b++) {
			e.select(c[b], true, a)
		}
	},
...
Ext.define("Ext.util.Filter", {
	...
	createValueMatcher : function() {
		var d = this, e = d.getValue(), f = d.getAnyMatch(), c = d
				.getExactMatch(), a = d.getCaseSensitive(), b = Ext.String.escapeRegex;
...
Ext.define("Ext.util.Point", {
	...
	isCloseTo : function(c, b) {
		...
		return (this.x <= a + e && this.x >= a - e && this.y <= f + d && this.y >= f
				- d)
	},
	isWithin : function() {
...
Ext.define("Ext.util.Region", {
	...
	intersect : function(g) {
		var f = this, d = Math.max(f.top, g.top), e = Math
				.min(f.right, g.right), a = Math.min(f.bottom, g.bottom), c = Math
				.max(f.left, g.left);
		if (a > d && e > c) {
	...
	union : function(g) {
		var f = this, d = Math.min(f.top, g.top), e = Math
				.max(f.right, g.right), a = Math.max(f.bottom, g.bottom), c = Math
				.min(f.left, g.left);
		return new Ext.util.Region(d, e, a, c)
	},
...
Ext.define("Ext.util.Sorter", {
			...
			createSortFunction : function(b) {
				var c = this, a = c.getDirection().toUpperCase() == "DESC"
						? -1
						: 1;
				return function(e, d) {
			...
			updateSortFn : function() {
				this.sort = this.createSortFunction(this.getSorterFn()
						|| this.defaultSortFn)
			},
			toggle : function() {
...
Ext.define("Ext.util.Inflector", {
	singleton : true,
	plurals : [[(/(quiz)$/i), "$1zes"], [(/^(ox)$/i), "$1en"],
			[(/([m|l])ouse$/i), "$1ice"],
			[(/(matr|vert|ind)ix|ex$/i), "$1ices"],
			[(/(x|ch|ss|sh)$/i), "$1es"], [(/([^aeiouy]|qu)y$/i), "$1ies"],
			[(/(hive)$/i), "$1s"], [(/(?:([^f])fe|([lr])f)$/i), "$1$2ves"],
			[(/sis$/i), "ses"], [(/([ti])um$/i), "$1a"],
			[(/(buffal|tomat|potat)o$/i), "$1oes"], [(/(bu)s$/i), "$1ses"],
			[(/(alias|status|sex)$/i), "$1es"], [(/(octop|vir)us$/i), "$1i"],
			[(/(ax|test)is$/i), "$1es"], [(/^person$/), "people"],
			[(/^man$/), "men"], [(/^(child)$/), "$1ren"], [(/s$/i), "s"],
			[(/$/), "s"]],
	singulars : [
			[(/(quiz)zes$/i), "$1"],
			[(/(matr)ices$/i), "$1ix"],
			...
			[
					...
					"$1$2sis"], [(/([ti])a$/i), "$1um"],
			[(/(n)ews$/i), "$1ews"], [(/people$/i), "person"], [(/s$/i), ""]],
...
Ext.anims = {
	fade : new Ext.Anim({
				type : "fade",
				before : function(b) {
					var c = 1, a = 1, e = b.getStyle("z-index") == "auto"
							? 0
							: b.getStyle("z-index"), d = e;
					if (this.out) {
						a = 0
					} else {
						d = Math.abs(e) + 1;
	...
	slide : new Ext.Anim({
		...
		"z-index" : false,
		before : function(a) {
			var c = a.getStyle("z-index") == "auto" ? 0 : a.getStyle("z-index"), f = a
					.getStyle("opacity"), h = c + 1, d = this.out, l = this.direction, k = 0, i = 0, j = 0, g = 0, b = a
					.getHeight(), e = a.getWidth();
			if (l == "left" || l == "right") {
				if (d) {
					k = -e
				} else {
			...
			this.from = {
				"-webkit-transform" : "translate3d(" + j + "px, " + g
						+ "px, 0)",
				"z-index" : h,
				opacity : f - 0.01
			};
			this.to = {
				"-webkit-transform" : "translate3d(" + k + "px, " + i
						+ "px, 0)",
				"z-index" : h,
				opacity : f
			}
	...
	pop : new Ext.Anim({
				scaleOnExit : true,
				before : function(d) {
					var b = 1, c = 1, g = 1, a = 1, h = d.getStyle("z-index") == "auto"
							? 0
	...
	cube : new Ext.Anim({
		...
		style : "outer",
		before : function(b) {
			var k = "0% 0%", l = 0, a = 0, i = "Y", f = 0, g = 0, e = b
					.getWidth(), d = b.getHeight(), j = true, c = " translateX(0)", h = "";
			...
			this.from = {
				"-webkit-transform" : "rotate" + i + "(" + l + "deg)"
						+ (j ? " translateZ(" + f + "px)" : "") + c,
				"-webkit-transform-origin" : k
			};
			this.to = {
				"-webkit-transform" : "rotate" + i + "(" + a
						+ "deg) translateZ(" + g + "px)" + h,
				"-webkit-transform-origin" : k
			}
		},
		duration : 250
	...
	wipe : new Ext.Anim({
		before : function(b) {
			...
			if (!this.out) {
				...
				this.from = {
					"-webkit-mask-image" : a,
					"-webkit-mask-size" : b.getWidth() * 3 + "px "
							+ b.getHeight() + "px",
					"z-index" : c,
...
Ext.define("Ext.XTemplateCompiler", {
	...
	generate : function(a) {
		...
		c.switches = [];
		c.parse(a);
		c.funcs.push((c.useEval ? "$=" : "return") + " function (" + c.fnArgs
						+ ") {", c.body.join(""), "}");
		var b = c.funcs.join("\n");
		return b
	},
	...
	doFor : function(e, f) {
		var d = this, c = d.addFn(e), b = d.level, a = b - 1;
		d.body.push("var c", b, "=", c, d.callFn, ", a", b, "=Ext.isArray(c",
				b, "),p", b, "=(parent=c", a, "),r", b, "=values\n",
				"for (var i", b, "=0,n", b, "=a", b, "?c", b, ".length:(c", b,
				"?1:0), xcount=n", b, ";i", b, "<n" + b + ";++i", b, "){\n",
				"values=a", b, "?c", b, "[i", b, "]:c", b, "\n", "xindex=i", b,
				"+1\n")
	},
	doExec : function(c, d) {
		var b = this, a = "f" + b.funcs.length;
		b.funcs.push("function " + a + "(" + b.fnArgs + ") {",
				" try { with(values) {", "  " + c, " }} catch(e) {}", "}");
		b.body.push(a + b.callFn + "\n")
	},
	addFn : function(a) {
		var c = this, b = "f" + c.funcs.length;
		if (a === ".") {
			c.funcs.push("function " + b + "(" + c.fnArgs + ") {",
					" return values", "}")
		} else {
			if (a === "..") {
				c.funcs.push("function " + b + "(" + c.fnArgs + ") {",
	...
	parseTag : function(b) {
		var a = this.tagRe.exec(b), e = a[1], g = a[2], d = a[3], f = a[4], c;
		if (e == ".") {
...
Ext.define("Ext.data.writer.Json", {
			...
			writeRecords : function(d, e) {
				var a = this.getRootProperty(), f = d.getParams(), b = this
						.getAllowSingle(), c;
...
Ext.define("Ext.event.Dispatcher", {
			...
			doDispatchEvent : function(a, g, f, i, c, b) {
				var h = this.getListenerStack(a, g, f), d = this
						.getWildcardListenerStacks(a, g, f), e;
...
Ext.define("Ext.event.Touch", {
			...
			cloneTouches : function(f) {
				var e = this.touchesMap, h = [], d = null, b, c, g, a;
...
Ext.define("Ext.event.publisher.ComponentPaint", {
	...
	setDispatcher : function(a) {
		...
		a.doAddListener(b, "*", "renderedchange",
				"onBeforeComponentRenderedChange", this, null, "before");
		a.doAddListener(b, "*", "hiddenchange",
				"onBeforeComponentHiddenChange", this, null, "before");
		a.doAddListener(b, "*", "renderedchange", "onComponentRenderedChange",
				this, null, "after");
		a.doAddListener(b, "*", "hiddenchange", "onComponentHiddenChange",
				this, null, "after");
	...
	onBeforeComponentRenderedChange : function(b, d, g) {
		var f = this.eventNames, c = g ? f.painted : f.erased, e = this
				.getSubscribers(c), a;
		if (e && e.$length > 0) {
	...
	onBeforeComponentHiddenChange : function(c, d) {
		var f = this.eventNames, b = d ? f.erased : f.painted, e = this
				.getSubscribers(b), a;
		if (e && e.$length > 0) {
	...
	dispatchQueue : function(g) {
		var l = this.dispatcher, a = this.targetType, b = this.eventNames, e = g
				.slice(), f = e.length, c, k, h, d, j;
		g.length = 0;
		if (f > 0) {
			for (c = 0; c < f; c++) {
				k = e[c];
...
Ext.define("Ext.event.publisher.Dom", {
	...
	handledEvents : ["click", "focus", "blur", "mousemove", "mousedown",
			"mouseup", "mouseover", "mouseout", "keyup", "keydown", "keypress",
			"submit", "transitionend", "animationstart", "animationend"],
	classNameSplitRegex : /\s+/,
	SELECTOR_ALL : "*",
	constructor : function() {
	...
	subscribe : function(g, f) {
		...
		var e = g.match(this.idOrClassSelectorRegex), a = this
				.getSubscribers(f), c = a.id, d = a.className, b = a.selector, h, i;
		if (e !== null) {
			h = e[1];
			i = e[2];
			if (h === "#") {
	...
	getSubscribersCount : function(a) {
		...
		return this.getSubscribers(a).$length
				+ this.getSubscribers("*").$length
	}
});
Ext.define("Ext.event.publisher.TouchGesture", {
	...
	handles : function(a) {
		return this.callParent(arguments)
				|| this.eventToRecognizerMap.hasOwnProperty(a)
	},
	...
	processEvents : function() {
		var a = this.buffer, f = a.length, d = [], c, e, b;
		if (f > 0) {
			c = a.slice(0);
			a.length = 0;
			for (b = 0; b < f; b++) {
	...
	registerRecognizer : function(a) {
		var g = this.eventToRecognizerMap, e = this.activeRecognizers, c = a
				.getHandledEvents(), d, f, b;
	...
	onTouchStart : function(k) {
		var m = this.currentTargets, g = this.currentTouches, o = this.currentTouchesCount, n = k.changedTouches, f = k.touches, h = f.length, a = {}, l = n.length, d, c, j, b;
		o += l;
		if (o > h) {
			for (d = 0; d < h; d++) {
				c = f[d];
			...
			for (j in g) {
				if (g.hasOwnProperty(j)) {
					if (!a[j]) {
						...
						b = k.clone();
						c = g[j];
						c.targets = this.getBubblingTargets(this
								.getElementTarget(c.target));
						b.changedTouches = [c];
						this.onTouchEnd(b)
					}
				}
	...
	onTouchMove : function(j) {
		...
		var l = this.currentTargets, g = this.currentTouches, c = this
				.getMoveThrottle(), m = j.changedTouches, b = 0, f, k, d, n, a, h;
		j.setTargets(l);
	...
	onTouchEnd : function(d) {
		...
		var h = this.currentTargets, c = this.currentTouches, k = d.changedTouches, g = k.length, j, f, b, a;
		d.setTargets(h);
	...
	if (!Ext.feature.has.Touch) {
		this.override({
					...
					onEvent : function(d) {
						...
						var c = d.type, b = [d];
						if (c === "mousedown" && this.lastEventType
								&& this.lastEventType !== "mouseup") {
							var a = document.createEvent("MouseEvent");
							a.initMouseEvent("mouseup", d.bubbles,
									d.cancelable, document.defaultView,
									d.detail, d.screenX, d.screenY, d.clientX,
									d.clientY, d.ctrlKey, d.altKey, d.shiftKey,
									d.metaKey, d.metaKey, d.button,
									d.relatedTarget);
							this.onEvent(a)
						}
						if (c !== "mousemove") {
					...
					processEvent : function(a) {
						this.eventProcessors[this.map.mouseToTouch[a.type]]
								.call(this, a)
					}
				})
	}
...
Ext.define("Ext.event.recognizer.DoubleTap", {
	...
	onEnd : function(g) {
		var c = this, b = this.getMaxDuration(), h = g.changedTouches[0], f = g.time, a = this.lastTapTime, d;
		this.lastTapTime = f;
		if (a) {
			d = f - a;
			if (d <= b) {
...
Ext.define("Ext.event.recognizer.Drag", {
	...
	onTouchMove : function(d) {
		var c = d.changedTouches, f = c[0], a = f.point, b = d.time;
		if (this.lastPoint) {
		...
		if (!this.isStarted) {
			...
			this.fire("dragstart", d, this.startTouches, this.getInfo(d,
							this.startTouch))
		} else {
	...
	getInfo : function(j, i) {
		var d = j.time, a = this.startPoint, f = this.previousPoint, b = this.startTime, k = this.previousTime, l = this.lastPoint, h = l.x
				- a.x, g = l.y - a.y, c = {
			touch : i,
			startX : a.x,
			startY : a.y,
...
(function() {
	...
	Ext.DateExtras = {
		...
		isValid : function(n, c, l, j, f, g, e) {
			...
			g = g || 0;
			e = e || 0;
			var k = a.add(new Date(n < 100 ? 100 : n, c - 1, l, j, f, g, e),
					a.YEAR, n < 100 ? n - 100 : 0);
			return n == k.getFullYear() && c == k.getMonth() + 1
					&& l == k.getDate() && j == k.getHours()
					&& f == k.getMinutes() && g == k.getSeconds()
					&& e == k.getMilliseconds()
		},
		...
		createParser : (function() {
			...
			return function(l) {
				...
				a.parseFunctions[l] = Ext.functionFactory("input", "strict", b(
								c, e, f.join("")))
			}
		})(),
		parseCodes : {
		...
		getWeekOfYear : (function() {
			...
			return function(f) {
				var g = Date
						.UTC(f.getFullYear(), f.getMonth(), f.getDate() + 3)
						/ c, e = Math.floor(g / 7), h = new Date(e * d)
						.getUTCFullYear();
		...
		clearTime : function(e, i) {
			...
			e.setMilliseconds(0);
			if (e.getDate() != g) {
				for (var f = 1, h = a.add(e, Ext.Date.HOUR, f); h.getDate() != g; f++, h = a
						.add(e, Ext.Date.HOUR, f)) {
				}
				e.setDate(g);
		...
		add : function(g, f, h) {
			var i = Ext.Date.clone(g), c = Ext.Date;
			if (!f || h === 0) {
				return i
			}
				...
				case Ext.Date.MONTH :
					...
					if (e > 28) {
						e = Math.min(e, Ext.Date.getLastDateOfMonth(Ext.Date
										.add(Ext.Date.getFirstDateOfMonth(g),
												"mo", h)).getDate())
					}
					i.setDate(e);
...
Ext.define("Ext.fx.easing.BoundMomentum", {
	...
	getValue : function() {
		var a = this.getMomentum(), j = this.getBounce(), e = a
				.getStartVelocity(), f = e > 0 ? 1 : -1, g = this
				.getMinMomentumValue(), d = this.getMaxMomentumValue(), c = (f == 1)
				? d
				: g, h = this.lastValue, i, b;
		if (e === 0) {
...
Ext.define("Ext.fx.easing.EaseIn", {
	...
	getValue : function() {
		var c = Ext.Date.now() - this.getStartTime(), g = this.getDuration(), b = this
				.getStartValue(), a = this.getEndValue(), h = this.distance, e = c
				/ g, d = Math.pow(e, this.getExponent()), f = b + (d * h);
		if (c >= g) {
			this.isEnded = true;
			return a
		}
...
Ext.define("Ext.fx.easing.EaseOut", {
	...
	getValue : function() {
		var f = Ext.Date.now() - this.getStartTime(), d = this.getDuration(), b = this
				.getStartValue(), h = this.getEndValue(), a = this.distance, c = f
				/ d, g = 1 - c, e = 1 - Math.pow(g, this.getExponent()), i = b
				+ (e * a);
		if (f >= d) {
			this.isEnded = true;
			return h
...
Ext.define("Ext.mixin.Filterable", {
	...
	filter : function(a) {
		return this.getFilters().length ? Ext.Array.filter(a, this
						.getFilterFn()) : a
	},
...
Ext.define("Ext.mixin.Observable", {
	...
	doFireEvent : function(b, c, e, a) {
		...
		var f = this.getObservableId(), d = this.getEventDispatcher();
		return d.dispatchEvent(this.observableType, f, b, c, e, a)
	},
	doAddListener : function(a, i, k, l, c) {
		var e = (k && k !== this && k.isIdentifiable), b = this
				.getUsedSelectors(), f = b.$map, d = this.getObservableId(), g, j, h;
		if (!l) {
			l = {}
		}
		if (!k) {
	...
	addDispatcherListener : function(b, d, f, e, c, a) {
		return this.getEventDispatcher().addListener(this.observableType, b, d,
				f, e, c, a)
	},
	doRemoveListener : function(b, k, m, n, d) {
		var g = (m && m !== this && m.isIdentifiable), e = this
				.getObservableId(), a, l, f, h, c, j;
		if (n && n.delegate) {
			j = n.delegate;
			e += " " + j
		}
		...
		if (a && g) {
			...
			for (f = 0, h = l.length; f < h; f++) {
				c = l[f];
				if (c.fn === k && c.scope === m && c.delegate === j
						&& c.order === d) {
					l.splice(f, 1);
					break
				}
	...
	removeDispatcherListener : function(b, c, e, d, a) {
		return this.getEventDispatcher().removeListener(this.observableType, b,
				c, e, d, a)
	},
	...
	hasListener : function(a) {
		return this.getEventDispatcher().hasListener(this.observableType,
				this.getObservableId(), a)
	},
	...
	createEventRelayer : function(a) {
		return function() {
			return this.doFireEvent(a, Array.prototype.slice.call(arguments, 0,
							-2))
		}
	},
	...
	createEventBubbler : function(a) {
		return function b() {
			...
			if (c && c !== this && c.isObservable) {
				c.fireAction(a, Array.prototype.slice.call(arguments, 0, -2),
						b, c, null, "after")
			}
		}
	},
			...
			statics : {
				generateSetter : function(e) {
					...
					return function(h) {
						...
						if (h !== g) {
							if (i) {
								this.fireAction(a, [this, h, g], this.doSet,
										this, {
											nameMap : e
										})
							} else {
...
Ext.define("Ext.app.Controller", {
	...
	applyRoutes : function(a) {
		var f = this instanceof Ext.app.Application ? this : this
				.getApplication(), c = f.getRouter(), b, e, d;
		for (e in a) {
			b = a[e];
			d = {
	...
	hasRef : function(a) {
		return this.references
				&& this.references.indexOf(a.toLowerCase()) !== -1
	}
	,
}, function() {
});
Ext.define("Ext.app.History", {
			...
			constructor : function(a) {
				if (Ext.feature.has.History) {
					window.addEventListener("hashchange", Ext.bind(
									this.detectStateChange, this))
				} else {
			...
			detectStateChange : function() {
				var b = this.applyToken(window.location.hash), a = this
						.getToken();
				if (b != a) {
					this.onStateChange();
					this.setToken(b)
...
Ext.define("Ext.app.Profile", {
	...
	getDependencies : function() {
		var c = [], g = Ext.String.format, b = this.getApplication().getName(), d = this
				.getNamespace(), f = {
...
Ext.define("Ext.app.Application", {
	...
	requires : ["Ext.app.History", "Ext.app.Profile", "Ext.app.Router",
			"Ext.app.Action"],
	config : {
		profiles : [],
		controllers : [],
	...
	constructor : function(a) {
		...
		Ext.require(this.getRequires(), function() {
					if (this.getEnableLoader() !== false) {
						Ext.require(this.getProfiles(), this.onProfilesLoaded,
								this)
					}
				}, this)
	},
	dispatch : function(e, d) {
		...
		if (e) {
			var c = this.getCurrentProfile(), b = c
					? c.getNamespace()
					: undefined, a = this.getController(e.getController(), b);
			if (a) {
				if (d !== false) {
	...
	onProfilesLoaded : function() {
		var b = this.getProfiles(), e = b.length, g = [], d = this
				.gatherDependencies(), f, c, a;
		for (c = 0; c < e; c++) {
			g[c] = Ext.create(b[c], {
						application : this
					});
	...
	gatherDependencies : function() {
		var a = this.getModels().concat(this.getViews()).concat(this
				.getControllers());
...
Ext.define("Ext.data.Connection", {
	...
	request : function(j) {
		j = j || {};
		var f = this, i = j.scope || window, e = j.username || f.getUsername(), g = j.password
				|| f.getPassword() || "", b, c, d, a, h;
	...
	upload : function(e, c, i, k) {
		...
		Ext.fly(g).on("load",
				Ext.Function.bind(this.onUploadComplete, this, [g, k]), null, {
					single : true
				});
		e.submit();
		Ext.fly(e).set(f);
	...
	onUploadComplete : function(h, b) {
		...
		try {
			...
			if (g) {
				if (g.body) {
					if (this.textAreaRe
							.test((f = g.body.firstChild || {}).tagName)) {
	...
	isFormUpload : function(a) {
		...
		if (b) {
			return (a.isUpload || (this.multiPartRe).test(b
	...
	setOptions : function(k, j) {
		var h = this, e = k.params || {}, g = h.getExtraParams(), d = k.urlParams, c = k.url
				|| h.getUrl(), i = k.jsonData, b, a, f;
		if (Ext.isFunction(e)) {
			e = e.call(j, k)
		}
		if (Ext.isFunction(c)) {
		...
		b = (k.method || h.getMethod() || ((e || f) ? "POST" : "GET"))
				.toUpperCase();
		this.setupMethod(k, b);
		a = k.disableCaching !== false ? (k.disableCaching || h
				.getDisableCaching()) : false;
		if (b === "GET" && a) {
			c = Ext.urlAppend(c, (k.disableCachingParam || h
	...
	setupHeaders : function(l, m, d, c) {
		var h = this, b = Ext.apply({}, m.headers || {}, h.getDefaultHeaders()
						|| {}), k = h.getDefaultPostHeader(), i = m.jsonData, a = m.xmlData, j, f;
		...
		if (((h.getUseDefaultXhrHeader() && m.useDefaultXhrHeader !== false) || m.useDefaultXhrHeader)
				&& !b["X-Requested-With"]) {
	...
	parseStatus : function(a, d) {
		...
		var c = (a >= 200 && a < 300) || a == 304
				|| (a == 0 && d.responseText.length > 0), b = false;
		if (!c) {
			switch (a) {
				case 12002 :
				case 12029 :
...
Ext.define("Ext.data.reader.Reader", {
	...
	buildExtractors : function() {
		...
		var b = this, c = b.getTotalProperty(), a = b.getSuccessProperty(), d = b
				.getMessageProperty();
		if (c) {
	...
	buildRecordDataExtractor : function() {
		var k = this, e = k.getModel(), g = e.getFields(), j = g.length, a = [], h = k
				.getModel().getClientIdProperty(), f = "__field", b = [
				"var me = this,\n",
		...
		for (d = 0; d < j; d++) {
			...
			c = a[d];
			m = l.getName();
			if (m === e.getIdProperty() && l.getMapping() === null
					&& e.getIdProperty() !== this.getIdProperty()) {
				l.setMapping(this.getIdProperty())
			}
			b.push("        try {\n");
			b.push("            value = ", k.createFieldAccessExpression(l, c,
							"source"), ";\n");
		...
		if (h) {
			b.push("        internalId = "
					+ k.createFieldAccessExpression(Ext.create(
									"Ext.data.Field", {
										name : h
									}), null, "source") + ";\n");
	...
	extractData : function(l) {
		var j = this, e = [], c = l.length, h = j.getModel(), m = h
				.getIdProperty(), k = h.getFields(), d, g, f, b, a;
		if (k.isDirty) {
	...
	readAssociated : function(h, g) {
		var e = this.getModel().associations.items, d = 0, f = e.length, a, c, b;
		for (; d < f; d++) {
			a = e[d];
...
Ext.define("Ext.data.reader.Json", {
			...
			getResponseData : function(a) {
				...
				try {
					...
					this.fireEvent("exception", this, a,
							"Unable to parse the JSON returned by the server: "
									+ b.toString());
					Ext.Logger
			...
			createFieldAccessExpression : function(g, b, c) {
				var f = this, h = f.objectRe, e = (g.getMapping() !== null), a = e
						? g.getMapping()
						: g.getName(), i, d;
				if (typeof a === "function") {
					i = b + ".getMapping()(" + c + ", this)"
				} else {
					if (f.getUseSimpleAccessors() === true
							|| ((d = String(a).search(h)) < 0)) {
						if (!e || isNaN(a)) {
							a = '"' + a + '"'
						}
						i = c + "[" + a + "]"
...
Ext.define("Ext.data.proxy.Proxy", {
				...
				if (a) {
					...
					if (a.onMetaChange) {
						a.onMetaChange = Ext.Function.createSequence(
								a.onMetaChange, this.onMetaChange, this)
					}
				}
			},
			...
			batch : function(e, f) {
				var g = this, d = g.getBatchActions(), c = this.getModel(), b, a;
...
Ext.define("Ext.data.proxy.Server", {
	...
	buildRequest : function(a) {
		var c = this, d = Ext.applyIf(a.getParams() || {}, c.getExtraParams()
						|| {}), b;
	...
	getParams : function(i) {
		var n = this, h = {}, a = i.getGrouper(), m = i.getSorters(), f = i
				.getFilters(), l = i.getPage(), d = i.getStart(), g = i
				.getLimit(), o = n.getSimpleSortMode(), q = n.getPageParam(), k = n
				.getStartParam(), p = n.getLimitParam(), j = n.getGroupParam(), e = n
				.getSortParam(), c = n.getFilterParam(), b = n
				.getDirectionParam();
	...
	buildUrl : function(c) {
		...
		if (b.getNoCache()) {
			a = Ext.urlAppend(a, Ext.String.format("{0}={1}", b
									.getCacheString(), Ext.Date.now()))
		}
		return a
	},
	getUrl : function(a) {
...
Ext.define("Ext.data.proxy.JsonP", {
			...
			buildUrl : function(f) {
				var h = this, a = h.callParent(arguments), e = Ext.apply({}, f
								.getParams()), c = e.filters, d, b, g, j;
				delete e.filters;
				if (h.getAutoAppendParams()) {
					a = Ext.urlAppend(a, Ext.Object.toQueryString(e))
				}
				if (c && c.length) {
					for (g = 0; g < c.length; g++) {
						...
						j = b.getValue();
						if (j) {
							a = Ext.urlAppend(a, b.getProperty() + "=" + j)
						}
					}
				}
				return a
...
Ext.define("Ext.data.proxy.WebStorage", {
	...
	read : function(e, j, k) {
		var c = [], a = this.getIds(), h = this.getModel(), l = h
				.getIdProperty(), d = e.getParams() || {}, b = a.length, f, g;
		if (d[l] !== undefined) {
			g = this.getRecord(d[l]);
			if (g) {
				c.push(g);
				e.setSuccessful()
			}
	...
	destroy : function(b, h, e) {
		var a = b.getRecords(), f = a.length, d = this.getIds(), g = []
				.concat(d), c;
		for (c = 0; c < f; c++) {
	...
	getRecord : function(d) {
		if (this.cache[d] === undefined) {
			var m = this.getRecordKey(d), n = this.getStorageObject()
					.getItem(m), g = {}, f = this.getModel(), k = f.getFields().items, e = k.length, h, l, c, j, a, b;
			if (!n) {
				return
			}
			a = Ext.decode(n);
	...
	setRecord : function(k, d) {
		if (d) {
			k.setId(d)
		} else {
			d = k.getId()
		}
		var m = this, a = k.getData(), j = {}, f = m.getModel(), l = f
				.getFields().items, e = l.length, h = 0, n, c, g, o, b;
		for (; h < e; h++) {
			n = l[h];
			c = n.getName();
			if (typeof n.getEncode() == "function") {
				j[c] = n.getEncode()(a[c], k)
			} else {
				if (n.getType().type == "date" && Ext.isDate(a[c])) {
					b = n.getDateFormat();
					if (b) {
						j[c] = Ext.Date.format(a[c], b)
					} else {
						j[c] = a[c].getTime()
					}
				} else {
		...
		g = m.getStorageObject();
		o = m.getRecordKey(d);
		m.cache[d] = k;
		g.removeItem(o);
		g.setItem(o, Ext.encode(j));
		k.commit()
	},
	...
	getIds : function() {
		var b = (this.getStorageObject().getItem(this.getId()) || "")
				.split(","), c = b.length, a;
		if (c == 1 && b[0] === "") {
			b = []
		}
	...
	setIds : function(b) {
		var c = this.getStorageObject(), d = b.join(","), e = this.getId(), a = this
				.getRecordCounterKey();
		c.removeItem(e);
		if (Ext.isEmpty(d)) {
			c.removeItem(a)
...
Ext.define("Ext.data.reader.Array", {
			...
			createFieldAccessExpression : function(g, c, b) {
				var f = this, e = g.getMapping(), d = (e == null) ? f
						.getModel().getFields().indexOf(g) : e, a;
				if (typeof d === "function") {
...
Ext.define("Ext.data.reader.Xml", {
			...
			createFieldAccessExpression : function(e, d, c) {
				...
				if (typeof b === "function") {
					...
					a = 'me.getNodeValue(Ext.DomQuery.selectNode("' + b + '", '
							+ c + "))"
				}
				return a
			}
...
Ext.define("Ext.fx.animation.Abstract", {
	...
	getData : function() {
		var k = this.getStates(), e = {}, g = this.getBefore(), c = this
				.getAfter(), h = k[this.STATE_FROM], i = k[this.STATE_TO], j = h
				.getData(), f = i.getData(), d, b, a;
		for (b in k) {
...
Ext.define("Ext.fx.animation.Cube", {
	...
	getData : function() {
		var m = this.getTo(), n = this.getFrom(), k = this.getBefore(), a = this
				.getAfter(), e = this.getOut(), j = this.getDirection(), b = this
				.getElement(), g = b.getWidth(), c = b.getHeight(), l = e
				? "100% 100%"
				: "0% 0%", i = 1, d = 1, f = {
			rotateY : 0,
			translateZ : 0
...
Ext.define("Ext.fx.animation.Flip", {
	...
	getData : function() {
		var h = this.getFrom(), i = this.getTo(), g = this.getDirection(), b = this
				.getOut(), l = this.getHalf(), c = (l) ? 90 : 180, e = 1, a = 1, k = 0, j = 0, f = 0, d = 0;
		if (b) {
			a = 0.8
		} else {
			e = 0.8
...
Ext.define("Ext.fx.animation.Slide", {
	...
	getData : function() {
		var p = this.getElementBox(), c = this.getContainerBox(), g = p ? p : c, n = this
				.getFrom(), o = this.getTo(), f = this.getOut(), e = this
				.getOffset(), m = this.getDirection(), b = this
				.getUseCssTransform(), h = this.getReverse(), d = 0, a = 0, l, j, k, i;
		if (h) {
...
Ext.define("Ext.fx.Animation", {
			requires : ["Ext.fx.animation.Slide", "Ext.fx.animation.SlideOut",
					"Ext.fx.animation.Fade", "Ext.fx.animation.FadeOut",
					"Ext.fx.animation.Flip", "Ext.fx.animation.Pop",
...
Ext.define("Ext.fx.layout.card.Style", {
			...
			onActiveItemChange : function(a, e, h, i, d) {
				...
				if (e && h && h.isPainted()) {
					...
					f.dom.style.setProperty("visibility", "hidden",
							"!important");
					e.show();
...
Ext.define("Ext.fx.layout.card.Flip", {
			...
			updateDuration : function(d) {
				var c = d / 2, b = this.getInAnimation(), a = this
						.getOutAnimation();
				b.setDelay(c);
				b.setDuration(c);
				a.setDuration(c)
...
Ext.define("Ext.fx.layout.Card", {
			requires : ["Ext.fx.layout.card.Slide", "Ext.fx.layout.card.Cover",
					"Ext.fx.layout.card.Reveal", "Ext.fx.layout.card.Fade",
					"Ext.fx.layout.card.Flip", "Ext.fx.layout.card.Pop",
...
Ext.define("Ext.fx.runner.Css", {
	...
	constructor : function() {
		...
		if (a) {
			this.transformMethods = ["translateX", "translateY", "translateZ",
					"rotate", "rotateX", "rotateY", "rotateZ", "skewX",
					"skewY", "scaleX", "scaleY", "scaleZ"]
		} else {
			this.transformMethods = ["translateX", "translateY", "rotate",
...
Ext.define("Ext.fx.runner.CssTransition", {
	...
	attachListeners : function() {
		...
		this.getEventDispatcher().addListener("element", "*", "transitionend",
	...
	run : function(p) {
		var F = this, h = this.lengthProperties, x = {}, E = {}, G = {}, d, s, y, e, u, I, v, q, r, a, A, z, o, B, l, t, g, C, H, k, f, w, n, c, D, b, m;
		...
		for (A = 0, o = p.length; A < o; A++) {
			...
			b = a.sessions;
			if (b.length > 0) {
				this.refreshRunningAnimationsData(d, Ext.Array.merge(I, v),
						true, G.replacePrevious)
			}
			c = a.nameMap;
			D = a.nameList;
			t = {};
		...
		for (h in f) {
			if (f.hasOwnProperty(h)) {
				...
				for (b = 0, c = g.length; b < c; b++) {
					e = g[b];
					if (e.animation === d) {
						this.refreshRunningAnimationsData(e.element, e.list
										.slice(), true)
					}
				}
			}
...
(function(a) {
	Ext.define("Ext.layout.Default", {
		extend : "Ext.Evented",
		alternateClassName : ["Ext.layout.AutoContainerLayout",
		...
		doItemFloatingChange : function(d, e) {
			var c = d.element, b = this.floatingItemCls;
			if (e) {
				if (d.getZIndex() === null) {
		...
		dockItem : function(f, b) {
			var c = this.container, g = f.renderElement, e = f.element, d = this.dockingInnerElement;
			if (!d) {
		...
		getDockingWrapper : function(b) {
			var e = this.currentDockingDirection, d = this.positionDirectionMap[b], c = this.dockingWrapper;
			if (e !== d) {
		...
		createCenteringWrapper : function(c) {
			var f = c.getId(), d = this.centeringWrappers, b = c.renderElement, e;
			d[f] = e = b.wrap({
		...
		destroyCenteringWrapper : function(c) {
			var f = c.getId(), d = this.centeringWrappers, b = c.renderElement, e = d[f];
			b.unwrap();
			e.destroy();
			delete d[f];
			return this
		...
		insertItem : function(k, g) {
			var d = this.container, j = d.getItems().items, e = this.innerItems, c = this
					.getInnerItemsContainer().dom, i = k.renderElement.dom, h, f, b;
			if (d.has(k)) {
...
Ext.define("Ext.layout.AbstractBox", {
			...
			doItemAdd : function(d, b) {
				this.callParent(arguments);
				if (d.isInnerItem()) {
					var c = d.getConfig(this.sizePropertyName), a = d.config;
					if (!c && ("flex" in a)) {
			...
			redrawContainer : function() {
				var a = this.container, b = a.renderElement.dom.parentNode;
				if (b && b.nodeType !== 11) {
					a.innerElement.redraw()
				}
			},
			setItemFlex : function(c, a) {
				var b = c.element, d = this.flexItemCls;
				if (a) {
					b.addCls(d)
				} else {
					if (b.hasCls(d)) {
...
Ext.define("Ext.layout.Layout", {
			requires : ["Ext.layout.Fit", "Ext.layout.Card", "Ext.layout.HBox",
					"Ext.layout.VBox"],
			constructor : function(a, b) {
				var c = Ext.layout.Default, d, e;
				if (typeof b == "string") {
					d = b;
					b = {}
				} else {
...
Ext.define("Ext.mixin.Sortable", {
			...
			insertSorters : function(e, h, a) {
				...
				var f = h.length, j = a || this.getDefaultSortDirection(), c = this
						.getSortRoot(), k = this.getSorters(), l = [], g, b, m, d;
				if (!k) {
...
Ext.define("Ext.util.Collection", {
	...
	addAll : function(h) {
		var q = this, e = q.filtered, a = q.sorted, b = q.all, k = q.items, j = q.keys, p = q.map, l = q
				.getAutoFilter(), m = q.getAutoSort(), r = [], f = [], c = q.mixins.filterable, d = [], g, s, n, o;
		if (Ext.isObject(h)) {
			for (s in h) {
	...
	insert : function(c, d, f) {
		...
		this.all.push(f);
		if (b && this.getAutoFilter()
				&& this.mixins.filterable.isFiltered.call(e, f)) {
			return null
		}
		e.length++;
	...
	insertAll : function(g, d) {
		...
		var s = this, h = this.filtered, a = this.sorted, b = this.all, m = this.items, l = this.keys, r = this.map, n = this
				.getAutoFilter(), o = this.getAutoSort(), t = [], j = [], f = [], c = this.mixins.filterable, e = false, k, u, p, q;
...
Ext.define("Ext.data.proxy.Direct", {
			...
			applyDirectFn : function(a) {
				return Ext.direct.Manager.parseMethod(a)
			},
			applyApi : function(b) {
				var a;
				if (b && Ext.isObject(b)) {
					for (a in b) {
						if (b.hasOwnProperty(a)) {
							b[a] = Ext.direct.Manager.parseMethod(b[a])
						}
					}
				}
				return b
			...
			doRequest : function(d, j, k) {
				var h = this, b = h.getWriter(), e = h.buildRequest(d, j, k), f = h
						.getApi(), i = f && f[e.getAction()] || h.getDirectFn(), c = e
						.getParams(), g = [], a;
				e = b.write(e);
				if (d.getAction() == "read") {
			...
			createRequestCallback : function(d, a, e, b) {
				var c = this;
				return function(g, f) {
					c.processResponse(f.getStatus(), a, d, f.getResult(), e, b)
				}
			},
...
Ext.define("Ext.direct.RemotingEvent", {
			...
			getTransaction : function() {
				return this._transaction
						|| Ext.direct.Manager.getTransaction(this.getTid())
			}
		});
...
Ext.define("Ext.util.Geolocation", {
				...
				if (Ext.feature.has.Geolocation) {
					if (!a) {
						if (navigator && navigator.geolocation) {
							...
							if (window.google) {
								a = google.gears.factory
										.create("beta.geolocation")
							}
						}
					}
				} else {
					this.fireEvent("locationerror", this, false, false, true,
			...
			updateAutoUpdate : function(a, b) {
				...
				if (a) {
					if (!f) {
						c.fireEvent("locationerror", c, false, false, true,
								null);
						return
					}
					try {
			...
			updateWatchOperation : function() {
				...
				function c() {
					b.getCurrentPosition(Ext.bind(a.fireUpdate, a), Ext.bind(
									a.fireError, a), a.parseOptions())
				}
				c();
			...
			fireError : function(a) {
				var b = a.code;
				this.fireEvent("locationerror", this, b == a.TIMEOUT,
						b == a.PERMISSION_DENIED, b == a.POSITION_UNAVAILABLE,
...
Ext.define("Ext.data.NodeInterface", {
	...
	statics : {
		decorate : function(d) {
			if (!d.isNode) {
				var g = Ext.data.ModelManager, c = d.modelName, e = g
						.getModel(c), b = [], f, h, a;
		...
		getPrototypeBody : function() {
			return {
				...
				updateInfo : function(g) {
					var j = this, f = j.parentNode, b = (!f
							? true
							: f.firstChild == j), e = (!f
							? true
							: f.lastChild == j), d = 0, k = j, a = j.childNodes, h = a.length, c;
					while (k.parentNode) {
						++d;
						k = k.parentNode
					}
...
Ext.define("Ext.data.association.Association", {
	...
	statics : {
		create : function(a) {
			if (!a.isAssociation) {
				...
				return Ext.factory(a, Ext.data.association.Association, null,
						"association")
			}
			return a
		}
	...
	applyOwnerModel : function(a) {
		...
		if (b === undefined) {
			Ext.Logger
					.error("The configured ownerModel was not valid (you tried "
							+ a + ")")
		}
		return b
	},
	...
	applyAssociatedModel : function(a) {
		...
		if (b === undefined) {
			Ext.Logger
					.error("The configured associatedModel was not valid (you tried "
							+ a + ")")
		}
		return b
	},
...
Ext.define("Ext.data.association.BelongsTo", {
			...
			createSetter : function() {
				...
				return function(g, e, f) {
					...
					if (c) {
						g = Ext.data.Model.cache[Ext.data.Model
								.generateCacheId(c.getOwnerModel().modelName, g)];
						if (g) {
			...
			createGetter : function() {
				var c = this, d = c.getAssociatedModel(), b = c.getForeignKey(), a = c
						.getInstanceName();
				return function(h, i) {
					h = h || {};
					...
					e = g[a];
					if (!e) {
						e = Ext.data.Model.cache[Ext.data.Model
								.generateCacheId(d.modelName, j)];
						if (e) {
							g[a] = e
						}
					}
			...
			getInverseAssociation : function() {
				var b = this.getOwnerModel().modelName, a = this
						.getForeignKey();
				return this.getAssociatedModel().associations.findBy(
						function(d) {
							var c = d.getType().toLowerCase();
							return (c === "hasmany" || c === "hasone")
									&& d.getAssociatedModel().modelName === b
									&& d.getForeignKey() === a
						})
			}
		});
Ext.define("Ext.data.association.HasMany", {
	...
	applyName : function(a) {
		if (!a) {
			a = Ext.util.Inflector.pluralize(this.getAssociatedName()
					.toLowerCase())
		}
		return a
	},
	...
	applyStore : function(a) {
		var e = this, c = e, i = e.getAssociatedModel(), f = e.getStoreName(), d = e
				.getForeignKey(), h = e.getPrimaryKey(), g = e
				.getFilterProperty(), b = e.getAutoLoad();
		return function() {
			var m = this, k, l, j = {};
			if (m[f] === undefined) {
				if (g) {
	...
	getInverseAssociation : function() {
		...
		return this.getAssociatedModel().associations.findBy(function(b) {
					return b.getType().toLowerCase() === "belongsto"
...
Ext.define("Ext.data.association.HasOne", {
			...
			createSetter : function() {
				var c = this, b = c.getForeignKey(), a = c.getInstanceName(), d = c
						.getAssociatedModel();
				return function(h, f, g) {
			...
			read : function(c, a, e) {
				var b = this.getInverseAssociation(), d = a.read([e])
						.getRecords()[0];
...
Ext.define("Ext.util.LineSegment", {
	...
	intersects : function(l) {
		var o = this.point1, m = this.point2, i = l.point1, f = l.point2, c = o.x, b = m.x, a = i.x, q = f.x, p = o.y, n = m.y, k = i.y, h = f.y, g = (c - b)
				* (k - h) - (p - n) * (a - q), j, e;
		if (g == 0) {
			return null
		}
		...
		if (j < Math.min(c, b) || j > Math.max(c, b) || j < Math.min(a, q)
				|| j > Math.max(a, q) || e < Math.min(p, n)
				|| e > Math.max(p, n) || e < Math.min(k, h)
				|| e > Math.max(k, h)) {
			return null
		}
...
Ext.define("Ext.util.SizeMonitor", {
	...
	constructor : function(d) {
		this.initConfig(d);
		this.doFireSizeChangeEvent = Ext.Function.bind(
				this.doFireSizeChangeEvent, this);
		var g = this, e = this.getElement().dom, b = this.getDetectorCls(), c = Ext.Element
				.create({
							classList : [b, b + "-expand"],
							children : [{}]
						}, true), h = Ext.Element.create({
	...
	refreshPosition : function(b) {
		var e = this.detectors[b], a = this.position[b], d, c;
	...
	onDetectorScroll : function(b) {
		var c = this.detectors[b], a = this.position[b];
	...
	destroyDetector : function(a) {
		var c = this.detectors[a], b = this.listeners[a];
...
Ext.define("Ext.event.publisher.ComponentSize", {
	...
	subscribe : function(g) {
		var c = g.match(this.idSelectorRegex), f = this.subscribers, a = this.sizeMonitors, d = this.dispatcher, e = this.targetType, b;
		if (!c) {
			return false
		}
		if (!f.hasOwnProperty(g)) {
			f[g] = 0;
			d.addListener(e, g, "painted", "onComponentPainted", this, null,
					"before");
	...
	unsubscribe : function(h, b, e) {
		var c = h.match(this.idSelectorRegex), g = this.subscribers, d = this.dispatcher, f = this.targetType, a = this.sizeMonitors;
		if (!c) {
			return false
		}
...
Ext.define("Ext.data.proxy.Ajax", {
			...
			doRequest : function(a, e, b) {
				var d = this.getWriter(), c = this.buildRequest(a);
				c.setConfig({
...
Ext.define("Ext.data.Model", {
	...
	isModel : true,
	requires : ["Ext.util.Collection", "Ext.data.Field",
			"Ext.data.identifier.Simple", "Ext.data.ModelManager",
			"Ext.data.proxy.Ajax", "Ext.data.association.HasMany",
	...
	staticConfigs : ["idProperty", "fields", "validations", "associations",
			"hasMany", "hasOne", "belongsTo", "clientIdProperty", "identifier",
			"proxy"],
	statics : {
		EDIT : "edit",
		REJECT : "reject",
	...
	inheritableStatics : {
		load : function(a, b, h) {
			...
			if (!f) {
				Ext.Logger
						.error("You are trying to load a model that doesn't have a Proxy specified")
			}
			g = function(j) {
				if (j.wasSuccessful()) {
					e = j.getRecords()[0];
	...
	constructor : function(e, g, b, f) {
		var d = this, c = null, a = this.getIdProperty();
		d.modified = {};
		d.raw = b || e || {};
		d.stores = [];
		e = e || f || {};
	...
	mergeData : function(a) {
		var g = this, d = g.getFields().items, e = d.length, b = g.data, c, h, k, j, f;
		for (c = 0; c < e; c++) {
			h = d[c];
			k = h.getName();
			f = h.getConvert();
	...
	setData : function(a) {
		var g = this.fields.items, h = g.length, f = Ext.isArray(a), d = this._data = this.data = {}, e, k, b, l, j, c;
		if (!a) {
			return this
		}
		for (e = 0; e < h; e++) {
	...
	set : function(p, l) {
		var j = this, b = j.fields.map, o = j.modified, a = !j.editing, c = j.associations.items, f = 0, d = [], k, n, e, m, g, h;
		if (arguments.length == 1) {
			for (n in p) {
	...
	endEdit : function(a, c) {
		...
		if (b.editing) {
			...
			if (a !== true && (b.changedWhileEditing())) {
				b.afterEdit(c || Ext.Object.getKeys(this.modified),
						this.modified)
			}
	...
	save : function(b, d) {
		...
		if (!c) {
			Ext.Logger
					.error("You are trying to save a model instance that doesn't have a Proxy specified")
		}
		b = b || {};
		d = d || e;
		if (Ext.isFunction(b)) {
	...
	erase : function(b, d) {
		...
		if (!c) {
			Ext.Logger
					.error("You are trying to erase a model instance that doesn't have a Proxy specified")
		}
		b = b || {};
		d = d || e;
		if (Ext.isFunction(b)) {
	...
	notifyStores : function(e) {
		var c = Ext.Array.clone(arguments), a = this.stores, f = a.length, d, b;
		c[0] = this;
		for (d = 0; d < f; ++d) {
			b = a[d];
	...
	copy : function(c) {
		var d = this, b = d.getIdProperty(), a = Ext.apply({}, d.raw), e = Ext
				.apply({}, d.data);
		delete a[b];
		delete e[b];
	...
	prepareAssociatedData : function(o, a, n) {
		var h = o.associations.items, l = h.length, e = {}, f, s, g, q, r, d, c, m, k, p, b;
		for (m = 0; m < l; m++) {
			d = h[m];
			s = d.getName();
			p = d.getType();
			...
			if (b && p.toLowerCase() == "hasmany") {
				...
				if (f && f.getCount() > 0) {
					...
					for (k = 0; k < r; k++) {
						...
						if (Ext.Array.indexOf(a, c) == -1) {
							a.push(c);
							e[s][k] = q.getData();
							Ext.apply(e[s][k], this.prepareAssociatedData(q, a,
											n))
						}
					}
				}
				...
				if (b
						&& (p.toLowerCase() == "belongsto" || p.toLowerCase() == "hasone")) {
					...
					if (q !== undefined) {
						...
						if (Ext.Array.indexOf(a, c) === -1) {
							...
							e[s] = q.getData();
							Ext
									.apply(e[s], this.prepareAssociatedData(q,
													a, n))
						}
					}
				}
	...
	addAssociations : function(e, h) {
		...
		for (d = 0, f = e.length; d < f; d++) {
			...
			Ext.ClassManager.onCreated(a, this,
					(typeof b.associatedModel === "string")
							? b.associatedModel
							: Ext.getClassName(b.associatedModel))
		}
	},
	...
	updateFields : function(c) {
		var d = c.length, e = this, h = e.self.prototype, j = this
				.getIdProperty(), a, f, g, b;
	...
	applyIdentifier : function(a) {
		...
		return Ext.factory(a, Ext.data.identifier.Simple, this.getIdentifier(),
				"data.identifier")
	},
	...
	onClassExtended : function(k, d, j) {
		var f = j.onBeforeCreated, b = this, h = b.prototype, e = Ext.Class.configNameCache, g = h.staticConfigs
				.concat(d.staticConfigs || []), c = h.config, a = d.config
				|| {}, i;
		d.config = a;
		j.onBeforeCreated = function(A, t) {
			var v = [], x = A.prototype, w = {}, m = x.config, n = g.length, q = [
					"set", "get"], s = q.length, o = m.associations || [], l = Ext
					.getClassName(A), z, y, r, p, u;
			for (r = 0; r < n; r++) {
				z = g[r];
				for (p = 0; p < s; p++) {
					y = e[z][q[p]];
			...
			Ext.require(v, function() {
				Ext.Function.interceptBefore(j, "onCreated", function() {
					...
					A.prototype.associations = A.associations = A.prototype._associations = (B && B.associations)
							? B.associations.clone()
							: new Ext.util.Collection(function(C) {
										return C.getName()
									});
					A.prototype.validations = A.validations = A.prototype._validations = (B && B.validations)
							? B.validations.clone()
							: new Ext.util.Collection(function(C) {
										return C.field
												? (C.field + "-" + C.type)
...
Ext.define("Ext.data.Store", {
	alias : "store.store",
	extend : "Ext.Evented",
	requires : ["Ext.util.Collection", "Ext.data.Operation",
			"Ext.data.proxy.Memory", "Ext.data.Model", "Ext.data.StoreManager",
			"Ext.util.Grouper"],
	statics : {
		create : function(a) {
			if (!a.isStore) {
	...
	applyModel : function(b) {
		...
		if (b && !b.prototype.isModel && Ext.isObject(b)) {
			b = Ext.data.ModelManager.registerType(b.storeId || b.id
							|| Ext.id(), b)
		}
		if (!b) {
			...
			if (a) {
				b = Ext.define("Ext.data.Store.ImplicitModel-"
								+ (this.getStoreId() || Ext.id()), {
							extend : "Ext.data.Model",
							config : {
								fields : a,
								proxy : this.getProxy()
	...
	insert : function(f, b) {
		...
		var j = this, l = false, d = this.data, g = b.length, a = this
				.getModel(), h = j.getModelDefaults(), k = false, c, e;
		b = b.slice();
		for (c = 0; c < g; c++) {
			e = b[c];
			if (!e.isModel) {
	...
	remove : function(b) {
		...
		var m = this, n = false, e = 0, a = this.getAutoSync(), q = m
				.getSyncRemovedRecords(), c = this.getDestroyRemovedRecords(), l = b.length, p = [], h = [], o, k = m.data.items, f, g, d;
		for (; e < l; e++) {
			f = b[e];
	...
	doRemoveAll : function(d) {
		var g = this, a = this.getDestroyRemovedRecords(), h = this
				.getSyncRemovedRecords(), c = g.data.all.slice(), f = c.length, e, b;
		for (e = 0; e < f; e++) {
			b = c[e];
			b.unjoin(g);
	...
	afterEdit : function(c, g, d) {
		var f = this, h = f.data, a = d[c.getIdProperty()] || c.getId(), b = h.keys
				.indexOf(a), e;
	...
	filter : function(e, d, f, a) {
		var c = this.data, b = e ? ((Ext.isFunction(e) || e.isFilter) ? e : {
			property : e,
			value : d,
			anyMatch : f,
			caseSensitive : a,
	...
	onProxyLoad : function(b) {
		var d = this, a = b.getRecords(), c = b.getResultSet(), e = b
				.wasSuccessful();
		if (c) {
			d.setTotalCount(c.getTotal())
		}
		if (e) {
			this.fireAction("datarefresh", [this, this.data, b],
					"doDataRefresh")
		}
		d.loaded = true;
		d.loading = false;
	...
	doDataRefresh : function(l, f, b) {
		var a = b.getRecords(), k = this, d = k.getDestroyRemovedRecords(), m = k
				.getSyncRemovedRecords(), c = f.all.slice(), h = k.removed, j = c.length, e, g;
	...
	getUpdatedRecords : function() {
		return this.data.filterBy(function(a) {
					return a.dirty === true && a.phantom !== true
							&& a.isValid()
				}).items
	},
...
Ext.define("Ext.data.NodeStore", {
			...
			createDataCollection : function() {
				...
				a.handleSort = Ext.Function.bind(this.handleTreeSort, this,
						[a], true);
				a.findInsertionIndex = Ext.Function.bind(
						this.handleTreeInsertionIndex, this, [a,
								a.findInsertionIndex], true);
				return a
			},
...
Ext.define("Ext.data.TreeStore", {
	...
	onNodeAppend : function(n, c) {
		var l = this.getProxy(), j = l.getReader(), b = this.getModel(), g = c.raw, d = [], a = j
				.getRootProperty(), m, h, f, k, e;
		if (!c.isLeaf()) {
			m = j.getRoot(g);
			if (m) {
				h = j.extractData(m);
	...
	onProxyLoad : function(b) {
		var d = this, a = b.getRecords(), e = b.wasSuccessful(), c = b
				.getNode();
		c.beginEdit();
		c.set("loading", false);
		if (e) {
...
Ext.define("Ext.data.proxy.Rest", {
	...
	buildUrl : function(f) {
		var i = this, e = f.getOperation(), c = e.getRecords() || [], h = c[0], g = i
				.getModel(), k = g.getIdProperty(), j = i.getFormat(), a = i
				.getUrl(f), d = f.getParams() || {}, b = (h && !h.phantom) ? h
				.getId() : d[k];
		if (i.getAppendId() && b) {
			if (!a.match(/\/$/)) {
				a += "/"
...
Ext.define("Ext.direct.RemotingProvider", {
	...
	extend : "Ext.direct.JsonProvider",
	requires : ["Ext.util.MixedCollection", "Ext.util.DelayedTask",
			"Ext.direct.Transaction", "Ext.direct.RemotingMethod"],
	config : {
		namespace : undefined,
		url : null,
	...
	createHandler : function(c, d) {
		...
		if (!d.getFormHandler()) {
			a = function() {
				b.configureRequest(c, d, Array.prototype.slice.call(arguments,
								0))
			}
		} else {
			a = function(f, g, e) {
	...
	runCallback : function(e, b) {
		var d = !!b.getStatus(), c = d ? "success" : "failure", f = e
				&& e.getCallback(), a;
		if (f) {
			a = b.getResult();
			if (Ext.isFunction(f)) {
	...
	getTransaction : function(a) {
		return a && a.getTid
				? Ext.direct.Manager.getTransaction(a.getTid())
				: null
	},
	configureRequest : function(d, a, f) {
		...
		b = Ext.create("Ext.direct.Transaction", {
					...
					method : a.getName(),
					data : e,
					callback : i && Ext.isFunction(h)
							? Ext.Function.bind(h, i)
							: h
				});
	...
	queueTransaction : function(c) {
		...
		if (a) {
			if (!b.callTask) {
				b.callTask = Ext.create("Ext.util.DelayedTask",
						b.combineAndSend, b)
			}
...
Ext.define("Ext.util.TapRepeater", {
			...
			onTouchStart : function(c) {
				...
				b.setTimer(Ext.defer(b.tap, b.getDelay() || b.getInterval(), b,
						[c]))
			},
			tap : function(b) {
				var a = this;
				a.fireEvent("tap", a, b);
				a.setTimer(Ext.defer(a.tap, a.getAccelerate() ? a.easeOutExpo(
								Ext.Date.getElapsed(a.tapStartTime), 400, -390,
								12000) : a.getInterval(), a, [b]))
			},
			easeOutExpo : function(e, a, g, f) {
				return (e == f) ? a + g : g * (-Math.pow(2, -10 * e / f) + 1)
						+ a
			},
...
Ext.define("Ext.util.translatable.Abstract", {
			...
			constructor : function(a) {
				var b;
				this.doAnimationFrame = Ext.Function.bind(
						this.doAnimationFrame, this);
				this.x = 0;
				this.y = 0;
			...
			animate : function(b, a) {
				...
				this.isAnimating = true;
				this.animationTimer = setInterval(this.doAnimationFrame,
			...
			translateAnimated : function(b, g, e) {
				...
				var d = Ext.Date.now(), f = e.easing, c = (typeof b == "number")
						? (e.easingX || this.getEasingX() || f || true)
						: null, a = (typeof g == "number") ? (e.easingY
						|| this.getEasingY() || f || true) : null;
				if (c) {
			...
			doAnimationFrame : function() {
				var c = this.activeEasingX, b = this.activeEasingY, d = this
						.getElement(), a, e;
...
Ext.define("Ext.util.translatable.CssTransform", {
			...
			doTranslate : function(a, c) {
				...
				b.webkitTransform = "translate3d(" + a + "px, " + c
						+ "px, 0px)";
				return this.callParent(arguments)
			},
			destroy : function() {
...
Ext.define("Ext.util.translatable.ScrollPosition", {
			...
			getWrapper : function() {
				var e = this.wrapper, c = this.baseCls, b = this.getElement(), d, a;
				if (!e) {
					a = b.getParent();
					if (!a) {
						return null
			...
			destroy : function() {
				var b = this.getElement(), a = this.baseCls;
				if (this.wrapper) {
					if (this.getUseWrapper()) {
						b.unwrap()
					}
...
Ext.define("Ext.util.Translatable", {
	...
	constructor : function(a) {
		var c = Ext.util.translatable, e = c.CssTransform, d = c.ScrollPosition, b;
...
Ext.define("Ext.scroll.Scroller", {
	extend : "Ext.Evented",
	requires : ["Ext.fx.easing.BoundMomentum", "Ext.fx.easing.EaseOut",
			"Ext.util.SizeMonitor", "Ext.util.Translatable"],
	config : {
		element : null,
		direction : "auto",
	...
	scrollToEnd : function(a) {
		return this
				.scrollTo(0, this.getSize().y - this.getContainerSize().y, a)
	},
	...
	onDragStart : function(l) {
		var o = this.getDirection(), g = l.absDeltaX, f = l.absDeltaY, j = this
				.getDirectionLock(), i = this.startPosition, d = this.flickStartPosition, k = this.flickStartTime, h = this.lastDragPosition, c = this.position, b = this.dragDirection, n = c.x, m = c.y, a = Ext.Date
				.now();
	...
	onAxisDrag : function(i, q) {
		...
		var h = this.flickStartPosition, l = this.flickStartTime, j = this.lastDragPosition, e = this.dragDirection, g = this.position[i], k = this
				.getMinPosition()[i], o = this.getMaxPosition()[i], d = this.startPosition[i], p = j[i], n = d
				- q, c = e[i], m = this.getOutOfBoundRestrictFactor(), f = this
				.getStartMomentumResetTime(), b = Ext.Date.now(), a;
		if (n < k) {
			n *= m
		} else {
			if (n > o) {
	...
	getAnimationEasing : function(g) {
		...
		var e = this.position[g], f = this.flickStartPosition[g], k = this.flickStartTime[g], c = this
				.getMinPosition()[g], j = this.getMaxPosition()[g], a = this
				.getMaxAbsoluteVelocity(), d = null, b = this.dragEndTime, l, i, h;
		if (e < c) {
			d = c
		} else {
			if (e > j) {
	...
	snapToSlot : function() {
		var b = this.getSnapPosition("x"), a = this.getSnapPosition("y"), c = this
				.getSlotSnapEasing();
		if (b !== null || a !== null) {
			this.scrollTo(b, a, {
						easingX : c.x,
	...
	snapToBoundary : function() {
		var g = this.position, c = this.getMinPosition(), f = this
				.getMaxPosition(), e = c.x, d = c.y, b = f.x, a = f.y, i = Math
				.round(g.x), h = Math.round(g.y);
		if (i < e) {
			i = e
		} else {
			if (i > b) {
	...
	destroy : function() {
		var b = this.getElement(), a = this.sizeMonitors;
		if (a) {
			a.element.destroy();
			a.container.destroy()
		}
...
Ext.define("Ext.util.Draggable", {
	...
	applyConstraint : function(a) {
		...
		if (a === "container") {
			return Ext.merge(this.getContainerConstraint(),
					this.extraConstraint)
		}
	...
	getContainerConstraint : function() {
		var b = this.getContainer(), c = this.getElement();
		if (!b || !c.dom) {
			return this.defaultConstraint
		}
		var h = c.dom, g = b.dom, d = h.offsetWidth, a = h.offsetHeight, f = g.offsetWidth, e = g.offsetHeight;
		return {
			min : {
				x : 0,
				y : 0
	...
	onDrag : function(b) {
		...
		this.fireAction("drag", [this, b, a.x + b.deltaX, a.y + b.deltaY],
				this.doDrag)
	},
...
(function(a) {
	Ext.define("Ext.Component", {
		...
		requires : ["Ext.ComponentManager", "Ext.XTemplate", "Ext.dom.Element",
				"Ext.behavior.Translatable", "Ext.behavior.Draggable"],
		xtype : "component",
		...
		triggerInitialized : function() {
			var c = this.onInitializedListeners, d = c.length, e, b;
			if (!this.initialized) {
				this.initialized = true;
				if (d > 0) {
					for (b = 0; b < d; b++) {
		...
		renderTo : function(b, d) {
			var f = this.renderElement.dom, e = Ext.getDom(b), c = Ext
					.getDom(d);
			if (e) {
				if (c) {
					e.insertBefore(f, c)
		...
		getInnerHtmlElement : function() {
			var b = this.innerHtmlElement, c = this.getStyleHtmlCls();
		...
		showBy : function(d, f) {
			...
			var b = Ext.Viewport, e = this.getParent();
		...
		alignTo : function(o, j) {
			var f = o.isComponent ? o.renderElement : o, c = this.renderElement, q = f
					.getPageBox(), B = this.getParent().element.getPageBox(), n = c
					.getPageBox(), y = q.height, p = q.width, t = n.height, v = n.width;
			B.bottom -= 5;
			B.height -= 10;
			B.left += 5;
			B.right -= 5;
			...
			var w = b[1].split(""), d = b[2].split(""), z = (b[3] === "?"), i = w[0], u = w[1]
					|| i, m = d[0], l = d[1] || m, r = q.top, e = q.left, k = y
					/ 2, g = p / 2, h = v / 2, s = t / 2, x, A;
			switch (i) {
				case "t" :
					switch (m) {
						case "c" :
...
Ext.define("Ext.Button", {
			...
			updateIconMask : function(a) {
				this.iconElement[a ? "addCls" : "removeCls"](this
						.getIconMaskCls())
			},
			...
			onPress : function() {
				var c = this, a = c.element, d = c.getPressedDelay(), b = c
						.getPressedCls();
				if (!c.getDisabled()) {
					if (d > 0) {
			...
			updateComponent : function(a, b) {
				if (b) {
					if (this.isRendered() && b.setRendered(false)) {
						b.fireAction("renderedchange", [this, b, false],
								"doUnsetComponent", this, {
									args : [b]
								})
					} else {
				...
				if (a) {
					if (this.isRendered() && a.setRendered(true)) {
						a.fireAction("renderedchange", [this, a, true],
								"doSetComponent", this, {
									args : [a]
								})
					} else {
...
Ext.define("Ext.Img", {
			...
			onLoad : function(a) {
				...
				if (this.getMode() === "background") {
					this.element.dom.style.backgroundImage = 'url("'
...
Ext.define("Ext.Map", {
			...
			updateMapOptions : function(d) {
				var a = this, c = (window.google || {}).maps, b = this.getMap();
				if (c && b) {
					b.setOptions(d)
				}
				if (d.center && !a.isPainted()) {
			...
			getMapOptions : function() {
				return Ext.merge({}, this.options
								|| this.getInitialConfig("mapOptions"))
			},
			...
			renderMap : function() {
				var d = this, f = (window.google || {}).maps, b = d.element, a = d
						.getMapOptions(), e = d.getMap(), c;
				if (f) {
					if (Ext.os.is.iPad) {
						Ext.merge({
					...
					e = d.getMap();
					c = f.event;
					c.addListener(e, "zoom_changed", Ext
									.bind(d.onZoomChange, d));
					c.addListener(e, "maptypeid_changed", Ext.bind(
									d.onTypeChange, d));
					c.addListener(e, "center_changed", Ext.bind(
									d.onCenterChange, d));
			...
			onGeoUpdate : function(a) {
				if (a) {
					this.setMapCenter(new google.maps.LatLng(a.getLatitude(), a
									.getLongitude()))
				}
			},
			onGeoError : Ext.emptyFn,
			setMapCenter : function(d) {
				var a = this, c = a.getMap(), b = (window.google || {}).maps;
				if (b) {
					if (!a.isPainted()) {
						a.un("painted", "setMapCenter", this);
						a.on("painted", "setMapCenter", this, {
...
Ext.define("Ext.Mask", {
			...
			updateTransparent : function(a) {
				this[a ? "addCls" : "removeCls"](this.getBaseCls()
						+ "-transparent")
			}
		});
Ext.define("Ext.LoadMask", {
	...
	refreshPosition : function() {
		var c = this.getParent(), d = c.getScrollable(), a = (d) ? d
				.getScroller() : null, f = (a) ? a.position : {
			x : 0,
			y : 0
		}, e = c.element.getSize(), b = this.element.getSize();
		this.innerElement.setStyle({
					marginTop : Math.round(e.height - b.height + (f.y * 2))
							+ "px",
...
Ext.define("Ext.Media", {
			...
			addMediaListener : function(d, b) {
				var c = this, e = c.media.dom, f = Ext.Function.bind;
				if (!Ext.isObject(d)) {
					var a = d;
					d = {};
					d[a] = b
...
Ext.define("Ext.carousel.Indicator", {
	...
	setActiveIndex : function(b) {
		var e = this.indicators, d = this.activeIndex, a = e[d], f = e[b], c = this
				.getBaseCls();
		if (a) {
	...
	onTap : function(f) {
		var g = f.touch, a = this.element.getPageBox(), d = a.left
				+ (a.width / 2), b = a.top + (a.height / 2), c = this
				.getDirection();
		if ((c === "horizontal" && g.pageX >= d)
				|| (c === "vertical" && g.pageY >= b)) {
...
Ext.define("Ext.dataview.IndexBar", {
			...
			onTouchMove : function(c) {
				var a = Ext.util.Point.fromEvent(c), b, d = this.pageBox;
				if (!d) {
			...
			destroy : function() {
				var c = this, d = Array.prototype.slice
						.call(c.innerElement.dom.childNodes), b = d.length, a = 0;
				for (; a < b; a++) {
					Ext.removeNode(d[a])
				}
				this.callParent()
...
Ext.define("Ext.dataview.element.Container", {
	...
	updateListItem : function(b, d) {
		var c = this, a = c.dataview, e = a.prepareData(b.getData(true), a
						.getStore().indexOf(b), b);
		d.innerHTML = c.dataview.getItemTpl().apply(e)
	},
	addListItem : function(e, c) {
		var h = this, d = h.dataview, a = d.prepareData(c.getData(true), d
						.getStore().indexOf(c), c), b = h.element, i = b.dom.childNodes, g = i.length, f;
...
Ext.define("Ext.dataview.element.List", {
	...
	getItemElementConfig : function(e, h) {
		...
		if (c.getOnItemDisclosure()) {
			d.children.push({
						cls : f.disclosureClsShortCache
								+ " "
								+ ((h[c.getDisclosureProperty()] === false)
										? f.hiddenDisplayCache
										: "")
					})
		}
		return d
	...
	updateListItem : function(d, k) {
		var h = this, e = h.dataview, j = Ext.fly(k), g = j.down(
				h.labelClsCache, true), c = e.prepareData(d.getData(true), e
						.getStore().indexOf(d), d), b = e
				.getDisclosureProperty(), a = c && c.hasOwnProperty(b), l = c
				&& c.hasOwnProperty("iconSrc"), f, i;
	...
	doRemoveHeaders : function() {
		var e = this, a = e.headerItemClsShortCache, b = e.element
				.query(e.headerClsCache), f = b.length, c = 0, d;
		for (; c < f; c++) {
			d = b[c];
	...
	doRemoveFooterCls : function() {
		var d = this, c = d.footerClsShortCache, a = d.element
				.query(d.footerClsCache), e = a.length, b = 0;
		for (; b < e; b++) {
...
Ext.define("Ext.event.publisher.ComponentDelegation", {
	...
	subscribe : function(g, f) {
		...
		var e = g.match(this.optimizedSelectorRegex), a = this.getSubscribers(
				f, true), k = a.type, c = a.selector, d, i, j, b, h;
		if (e !== null) {
			d = e[1];
			i = e[2].indexOf(">") === -1;
			j = e[3];
	...
	notify : function(d, a) {
		...
		if (b) {
			this.dispatcher.doAddListener(this.targetType, d, a, "publish",
					this, {
						args : [a, b]
					}, "before")
		}
	...
	publish : function(g, k) {
		...
		var p = arguments[arguments.length - 1], o = e.type, b = e.selector, d = Array.prototype.slice
				.call(arguments, 2, -2), l = k.xtypesChain, s, n, t, a, m, v, r, u, h, f, q, c;
...
Ext.define("Ext.field.Field", {
			...
			updateRequired : function(a) {
				this.renderElement[a ? "addCls" : "removeCls"](this
						.getRequiredCls())
			},
...
Ext.define("Ext.field.Checkbox", {
	...
	getSameGroupFields : function() {
		var j = this.up("formpanel") || this.up("fieldset"), b = this.getName(), g = this.qsaLeftRe, c = this.qsaRightRe, f = [], a, d, e, h;
		if (!j) {
			j = Ext.Viewport
		}
		b = b.replace(g, "\\[");
		b = b.replace(c, "\\]");
		a = Ext.query("[name=" + b + "]", j.element.dom);
		h = a.length;
		for (e = 0; e < h; e++) {
			d = a[e];
			d = Ext.fly(d).up(".x-field-" + d.getAttribute("type"));
			if (d && d.id) {
				f.push(Ext.getCmp(d.id))
			}
		}
		return f
	},
...
Ext.define("Ext.field.Text", {
	...
	syncEmptyCls : function() {
		var b = (this._value) ? this._value.length : false, a = Ext.baseCSSPrefix
				+ "empty";
		if (b) {
			this.removeCls(a)
		} else {
	...
	showClearIcon : function() {
		var a = this;
		if (!a.getDisabled() && !a.getReadOnly() && a.getValue()
				&& a.getClearIcon()) {
...
Ext.define("Ext.field.Spinner", {
			...
			updateComponent : function(c) {
				this.callParent(arguments);
				var b = this.innerElement, a = this.getCls();
				if (c) {
					...
					this.downRepeater = this.createRepeater(
							this.spinDownButton, this.onSpinDown);
					this.upRepeater = this.createRepeater(this.spinUpButton,
							this.onSpinUp)
				}
			},
			updateGroupButtons : function(a, e) {
				var c = this, d = c.innerElement, b = c.getBaseCls()
						+ "-grouped-buttons";
				c.getComponent();
				if (a != e) {
					if (a) {
			...
			spin : function(h) {
				var c = this, b = c.getValue(), a = c.getIncrement(), g = h
						? "down"
						: "up", e = c.getMinValue(), f = c.getMaxValue(), d;
				if (h) {
					d = b - a
				} else {
					d = b + a
			...
			destroy : function() {
				var a = this;
				Ext.destroy(a.downRepeater, a.upRepeater, a.spinDownButton,
						a.spinUpButton);
				a.callParent(arguments)
			}
		}, function() {
...
Ext.define("Ext.plugin.ListPaging", {
	...
	config : {
		...
		loadTpl : [
				...
				'<span class="{cssPrefix}loading-left"></span>', "</div>",
	...
	init : function(c) {
		...
		if (b) {
			this.disableDataViewMask(b)
		}
		c.updateStore = Ext.Function.createInterceptor(c.updateStore,
				this.bindStore, this);
	...
	disableDataViewMask : function(a) {
		var b = this.getList();
		if (a.isAutoLoading()) {
			b.setLoadingText(null)
		} else {
			a.on({
						load : {
							single : true,
	...
	onStoreLoad : function(a) {
		var d = this.addLoadMoreCmp(), b = this.getLoadTpl(), c = this
				.storeFullyLoaded() ? this.getNoMoreRecordsText() : this
				.getLoadMoreText();
	...
	storeFullyLoaded : function() {
		...
		return b !== null ? a.getTotalCount() <= (a.currentPage * a
				.getPageSize()) : false
	},
	loadNextPage : function() {
		...
		if (!a.storeFullyLoaded()) {
			a.setLoading(true);
			a.scrollY = a.getScroller().position.y;
...
Ext.define("Ext.plugin.PullRefresh", {
	...
	init : function(f) {
		var d = this, b = f.getStore(), e = d.getPullTpl(), c = d.element, a = f
		...
		e.overwrite(c, {
					message : d.getPullRefreshText(),
					lastUpdated : d.lastUpdated
				}, true);
		d.loadingElement = c.getFirstChild();
		d.messageEl = c.down(".x-list-pullrefresh-message");
		d.updatedEl = c.down(".x-list-pullrefresh-updated > span");
		d.maxScroller = a.getMaxPosition();
		a.on({
					maxpositionchange : d.setMaxScroller,
					scroll : d.onScrollChange,
					scope : d
				})
	},
	...
	setMaxScroller : function(b, a) {
		this.maxScroller = a
	},
	onScrollChange : function(b, a, c) {
		if (c < 0) {
			this.onBounceTop(c)
		}
		if (c > this.maxScroller.y) {
			this.onBounceBottom(c)
		}
	},
	applyPullTpl : function(a) {
		return (Ext.isObject(a) && a.isTemplate) ? a : new Ext.XTemplate(a)
	},
	onBounceTop : function(d) {
		var b = this, c = b.getList(), a = c.getScrollable().getScroller();
		if (!b.isReleased) {
			if (!b.isRefreshing && -d >= b.pullHeight + 10) {
				b.isRefreshing = true;
				b.setViewState("release");
				a.getContainer().onBefore({
	...
	onScrollerDragEnd : function() {
		var b = this;
		if (b.isRefreshing) {
			var c = b.getList(), a = c.getScrollable().getScroller();
			a.minPosition.y = -b.pullHeight;
			a.on({
						scrollend : "loadStore",
						single : true,
						scope : b
	...
	loadStore : function() {
		var b = this, c = b.getList(), a = c.getScrollable().getScroller();
		b.setViewState("loading");
		b.isReleased = false;
		Ext.defer(function() {
					a.on({
								scrollend : function() {
									if (b.getRefreshFn()) {
										b.getRefreshFn().call(b, b)
									} else {
										b.fetchLatest()
									}
									b.resetRefreshState()
					...
					a.minPosition.y = 0;
					a.scrollTo(null, 0, true)
				}, 500, b)
	},
	onBounceBottom : Ext.emptyFn,
	setViewState : function(d) {
		var b = this, c = Ext.baseCSSPrefix, e = b.messageEl, a = b.loadingElement;
		if (d === b.currentViewState) {
			return b
		}
		b.currentViewState = d;
		if (e && a) {
			switch (d) {
				case "pull" :
					e.setHtml(b.getPullRefreshText());
					a.removeCls([c + "list-pullrefresh-release",
							c + "list-pullrefresh-loading"]);
					break;
				case "release" :
					e.setHtml(b.getReleaseRefreshText());
					a.addCls(c + "list-pullrefresh-release");
					break;
				case "loading" :
					e.setHtml(b.getLoadingText());
					a.addCls(c + "list-pullrefresh-loading");
					break
			}
		}
		return b
	...
	resetRefreshState : function() {
		...
		a.setViewState("pull");
		a.updatedEl.setHtml(Ext.util.Format.date(a.lastUpdated, "m/d/Y h:iA"))
	}
});
Ext.define("Ext.scroll.indicator.Abstract", {
	...
	refresh : function() {
		var f = this.barElement, e = f.dom, c = this.getRatio(), b = this
				.getAxis(), a = (b === "x") ? e.offsetWidth : e.offsetHeight, d = a
				* c;
		this.barLength = a;
		this.gapLength = a - d;
		this.setLength(d);
...
Ext.define("Ext.scroll.indicator.CssTransform", {
	...
	updateLength : function(c) {
		var b = this.getAxis(), a = this.endElement.dom.style, e = this.middleElement.dom.style, d = this.endElementLength, g = c
				- d, f = g - this.startElementLength;
		if (b === "x") {
	...
	setOffset : function(c) {
		var a = this.getAxis(), b = this.element.dom.style;
		c = Math.round(c);
		if (a === "x") {
...
Ext.define("Ext.scroll.indicator.Default", {
	...
	setOffset : function(c) {
		var b = this.getAxis(), a = this.element.dom.style;
		if (b === "x") {
...
Ext.define("Ext.scroll.indicator.ScrollPosition", {
	...
	setLength : function(e) {
		var c = this.getAxis(), a = this.barLength, d = this.barElement.dom, b = this.element;
		this.callParent(arguments);
		if (c === "x") {
			d.scrollLeft = a;
			b.setLeft(a)
	...
	setOffset : function(d) {
		var b = this.getAxis(), a = this.barLength, c = this.barElement.dom;
		d = a - d;
		if (b === "x") {
			c.scrollLeft = d
		} else {
...
Ext.define("Ext.scroll.Indicator", {
			requires : ["Ext.scroll.indicator.Default",
...
Ext.define("Ext.scroll.View", {
	...
	updateElement : function(c) {
		var b = c.getFirstChild().getFirstChild(), a = this.getScroller();
	...
	setIndicatorValue : function(b, f) {
		...
		var a = this.getScroller(), c = a.getMaxPosition()[b], e = a
				.getContainerSize()[b], d;
		if (c === 0) {
			d = f / e;
			if (f >= 0) {
				d += 1
	...
	refreshIndicator : function(d) {
		...
		var a = this.getScroller(), b = this.getIndicators()[d], e = a
				.getContainerSize()[d], f = a.getSize()[d], c = e / f;
		b.setRatio(c);
		b.refresh()
	},
	refresh : function() {
...
Ext.define("Ext.behavior.Scrollable", {
			...
			setConfig : function(d) {
				var b = this.scrollView, c = this.component, e, a;
				if (d) {
					if (!b) {
...
Ext.define("Ext.Container", {
	...
	requires : ["Ext.layout.Layout", "Ext.ItemCollection",
			"Ext.behavior.Scrollable", "Ext.Mask"],
	xtype : "container",
	eventedConfig : {
		activeItem : 0
	},
	...
	refreshModalMask : function() {
		var b = this.getModal(), a = this.getParent();
		if (!this.painted) {
			this.painted = true;
			if (b) {
	...
	onFirstItemAdd : function() {
		...
		this.setLayout(new Ext.layout.Layout(this, this.getLayout()
						|| "default"));
	...
	factoryItemWithDefaults : function(c) {
		...
		if (c.isComponent) {
			...
			if (d && !c.ignoreDefaults) {
				if (!(c.hasOwnProperty("left") && c.hasOwnProperty("right")
						&& c.hasOwnProperty("top")
						&& c.hasOwnProperty("bottom")
						&& c.hasOwnProperty("docked") && c
						.hasOwnProperty("centered"))) {
					c = Ext.mergeIf({}, c, d)
				}
			}
	...
	add : function(a) {
		...
		for (b = 0; b < d; b++) {
			c = e.factoryItem(a[b]);
			this.doAdd(c);
			if (!f && !this.getActiveItem() && this.innerItems.length > 0
					&& c.isInnerItem()) {
				f = c
			}
		}
...
Ext.define("Ext.Panel", {
	...
	alignTo : function(m) {
		...
		this.callParent(arguments);
		var f = Ext.util.LineSegment, d = m.isComponent ? m.renderElement : m, a = this.renderElement, n = d
				.getPageBox(), k = a.getPageBox(), b = k.left, t = k.top, C = k.right, h = k.bottom, j = b
				+ (k.width / 2), i = t + (k.height / 2), o = {
			x : b,
			y : t
		}, l = {
			x : C,
			...
			x : j,
			y : i
		}, s = n.left + (n.width / 2), q = n.top + (n.height / 2), v = {
			x : s,
			y : q
		}, c = new f(y, v), g = 0, A = 0, e, z, r, p, x, u;
...
Ext.define("Ext.SegmentedButton", {
			...
			updateFirstAndLastCls : function(a) {
				var d = a.length, e = Ext.baseCSSPrefix, c, b;
				for (b = 0; b < d; b++) {
					c = a.items[b];
					if (!c.isHidden()) {
						c.addCls(e + "first");
			...
			updatePressedButtons : function(a) {
				var h = this, b = h.getItems(), g = h.getPressedCls(), f, d, e, c;
				e = b.length;
				for (c = 0; c < e; c++) {
					f = b.items[c];
					f.removeCls([g, f.getPressedCls()])
				}
				e = a.length;
				for (c = 0; c < e; c++) {
					d = a[c];
...
Ext.define("Ext.TitleBar", {
	...
	maxButtonWidth : "40%",
	constructor : function() {
		this.refreshTitlePosition = Ext.Function.createThrottled(
				this.refreshTitlePosition, 50, this);
		this.callParent(arguments)
	},
	beforeInitialize : function() {
	...
	applyInitialItems : function(a) {
		...
		b.titleComponent = b.add({
					xtype : "title",
					hidden : c.hidden,
					centered : true
				});
		b.doAdd = b.doBoxAdd;
		b.remove = b.doBoxRemove;
		b.doInsert = b.doBoxInsert
	},
	doBoxAdd : function(a) {
	...
	refreshTitlePosition : function() {
		...
		f.setWidth(null);
		f.setLeft(null);
		var a = this.leftBox, c = a.down("button"), h, m;
		if (c) {
			if (c.getWidth() == null) {
		...
		var j = this.spacer.renderElement.getPageBox(), k = f.getPageBox(), g = k.width
				- j.width, d = k.left, i = k.right, b, l, e;
		if (g > 0) {
			f.setWidth(j.width);
			b = g / 2;
			d += b;
...
Ext.define("Ext.MessageBox", {
	...
	constructor : function(a) {
		...
		var e = ["ui", "showAnimation", "hideAnimation", "title", "message",
				"prompt", "iconCls", "buttons", "defaultTextHeight"], d = e.length, b, c;
		for (b = 0; b < d; b++) {
			c = e[b];
	...
	applyPrompt : function(a) {
		if (a) {
			...
			if (b.multiLine) {
				b.height = Ext.isNumber(b.multiLine)
						? parseFloat(b.multiLine)
	...
	onClick : function(c) {
		if (c) {
			var b = c.config.userConfig || {}, d = c.getInitialConfig(), a = this
					.getPrompt();
			if (typeof b.fn == "function") {
				this.on({
							hiddenchange : function() {
								b.fn.call(b.scope || null, d.itemId || d.text,
										a ? a.getValue() : null, b)
							},
							single : true,
							scope : this
...
Ext.define("Ext.carousel.Carousel", {
	...
	xtype : "carousel",
	requires : ["Ext.fx.easing.EaseOut", "Ext.carousel.Item",
	...
	updateBufferSize : function(n) {
		var l = Ext.carousel.Item, h = n * 2 + 1, m = this.isRendered(), c = this.innerElement, g = this.carouselItems, f = g.length, e = this
				.getItemConfig(), d = this.getItemLength(), j = this
				.getDirection(), b = j === "horizontal"
				? "setWidth"
				: "setHeight", a, k;
		for (a = f; a < h; a++) {
			k = Ext.factory(e, l);
	...
	onDragStart : function(f) {
		var d = this.getDirection(), b = f.absDeltaX, a = f.absDeltaY, c = this
				.getDirectionLock();
		this.isDragging = true;
		if (c) {
	...
	onDrag : function(j) {
		...
		var k = this.dragStartOffset, l = this.getDirection(), m = l === "horizontal"
				? j.deltaX
				: j.deltaY, a = this.offset, i = this.flickStartTime, c = this.dragDirection, b = Ext.Date
				.now(), h = this.getActiveIndex(), f = this.getMaxItemIndex(), d = c, g;
		if ((h === 0 && m > 0) || (h === f && m < 0)) {
			m *= 0.5
		}
		g = k + m;
	...
	onDragEnd : function(j) {
		...
		this.onDrag(j);
		this.isDragging = false;
		var a = Ext.Date.now(), i = this.itemLength, g = i / 2, f = this.offset, m = this
				.getActiveIndex(), c = this.getMaxItemIndex(), h = 0, l = f
				- this.flickStartOffset, b = a - this.flickStartTime, k = this
				.getIndicator(), d;
	...
	setOffset : function(e) {
		var k = this.orderedCarouselItems, c = this.getBufferSize(), g = k[c], j = this.itemLength, d = this.currentAxis, a, h, b, f;
		this.offset = e;
		e += this.itemOffset;
		if (g) {
			g.translateAxis(d, e);
	...
	setOffsetAnimated : function(c) {
		var b = this.orderedCarouselItems[this.getBufferSize()], a = this
				.getIndicator();
		if (a) {
	...
	onActiveItemAnimationFrame : function(k) {
		var j = this.orderedCarouselItems, c = this.getBufferSize(), h = this.itemLength, d = this.currentAxis, e = k[d], g, a, f, b;
		for (f = 1, b = 0; f <= c; f++) {
			g = j[c - f];
			if (g) {
				b += h;
	...
	refreshSizing : function() {
		var a = this.element, b = this.getItemLength(), c, d;
	...
	doSetActiveItem : function(g) {
		var q = this.getActiveIndex(), e = this.getMaxItemIndex(), m = this
				.getIndicator(), c = this.getBufferSize(), k = this.carouselItems
				.slice(), n = this.orderedCarouselItems, p = {}, o = {}, a, l, b, f, h, j, d;
		if (k.length === 0) {
			return
		}
		this.callParent(arguments);
...
Ext.define("Ext.carousel.Infinite", {
	...
	updateBufferSize : function(b) {
		this.callParent(arguments);
		var e = b * 2 + 1, d = this.innerItems.length, c = this
				.getInnerItemConfig(), a;
		for (a = d; a < e; a++) {
	...
	rebuildInnerIndexes : function(n) {
		var c = this.innerIndexToItem, g = this.innerIdToIndex, j = this.innerItems
				.slice(), h = j.length, b = this.getBufferSize(), d = this
				.getMaxItemIndex(), l = [], e, k, f, a, m;
		if (n === undefined) {
...
Ext.define("Ext.dataview.component.DataItem", {
	...
	updateRecord : function(d) {
		...
		this._record = d;
		var h = this, c = h.config.dataview, b = c.prepareData(d.getData(true),
				c.getStore().indexOf(d), d), e = h.getItems(), k = e.first(), i = h
				.getDataMap(), f, j, g, a;
		if (!k) {
			return
		}
...
Ext.define("Ext.dataview.component.Container", {
	...
	moveItemsToCache : function(j, k) {
		var h = this, c = h.dataview, a = c.getMaxItemCache(), g = h
				.getViewItems(), f = h.itemCache, e = f.length, l = c
				.getPressedCls(), d = c.getSelectedCls(), b = k - j, m;
		for (; b >= 0; b--) {
			m = g[j + b];
			if (e !== a) {
				h.remove(m, false);
	...
	moveItemsFromCache : function(b) {
		var l = this, e = l.dataview, m = e.getStore(), k = b.length, a = e
				.getDefaultType(), h = e.getItemConfig(), g = l.itemCache, f = g.length, j = [], c, n, d;
		if (k) {
			e.hideEmptyText()
		}
		for (c = 0; c < k; c++) {
...
Ext.define("Ext.dataview.DataView", {
			...
			initialize : function() {
				...
				a = b.container = this
						.add(new Ext.dataview[b.getUseComponents()
								? "component"
								: "element"].Container({
			...
			doAddPressedCls : function(a) {
				var c = this, b = c.container.getViewItems()[c.getStore()
						.indexOf(a)];
				if (Ext.isElement(b)) {
					b = Ext.get(b)
				}
			...
			onItemTouchStart : function(b, h, d, g) {
				...
				f.fireAction("itemtouchstart", [f, d, h, a, g],
						"doItemTouchStart")
			},
			doItemTouchStart : function(c, b, e, a) {
				...
				if (a) {
					if (d > 0) {
						c.pressedTimeout = Ext.defer(c.doAddPressedCls, d, c,
								[a])
					} else {
						c.doAddPressedCls(a)
					}
			...
			applyItemTpl : function(a) {
				return (Ext.isObject(a) && a.isTemplate)
						? a
						: new Ext.XTemplate(a)
			},
			onAfterRender : function() {
			...
			doRefresh : function(f) {
				var a = f.container, j = f.getStore(), b = j.getRange(), e = a
						.getViewItems(), h = b.length, l = e.length, c = h - l, g = f
						.getScrollable(), d, k;
			...
			onStoreClear : function() {
				var c = this, a = c.container, b = a.getViewItems();
...
Ext.define("Ext.dataview.List", {
	...
	xtype : "list",
	requires : ["Ext.dataview.element.List", "Ext.dataview.IndexBar",
	...
	constructor : function() {
		this.translateHeader = (Ext.os.is.Android2)
				? this.translateHeaderCssPosition
				: this.translateHeaderTransform;
		this.callParent(arguments)
	},
	onItemTrigger : function(c, b, f, a, d) {
		if (!(this.getPreventSelectionOnDisclose() && Ext.fly(d.target)
				.hasCls(this.getBaseCls() + "-disclosure"))) {
			this.callParent(arguments)
		}
	},
	initialize : function() {
	...
	updateIndexBar : function(a) {
		if (a && this.getScrollable()) {
			this.indexBarElement = this.getScrollableBehavior().getScrollView()
					.getElement().appendChild(a.renderElement);
			a.on({
						index : "onIndex",
						scope : this
					});
	...
	updateGrouped : function(c) {
		var b = this.getBaseCls(), a = b + "-grouped", d = b + "-ungrouped";
		if (c) {
			this.addCls(a);
			this.removeCls(d);
	...
	createHeader : function() {
		var e, d = this.getScrollable(), a, b, c;
		if (d) {
			a = d.getScroller();
	...
	getClosestGroups : function() {
		var a = this.pinHeaderInfo.offsets, e = this.getScrollable(), d = a.length, b = 0, h, g, f, c;
		if (e) {
	...
	doRefreshHeaders : function() {
		...
		var l = this.findGroupHeaderIndices(), f = l.length, g = this.container
				.getViewItems(), j = this.pinHeaderInfo = {
			offsets : []
		}, a = j.offsets, h = this.getScrollable(), e, k, b, d, c;
		if (f) {
			for (b = 0; b < f; b++) {
				d = g[l[b]];
				if (d) {
	...
	onScroll : function(e, j, h) {
		var g = this, i = g.pinHeaderInfo, a = i.closest, b = g.activeGroup, c = i.headerHeight, d, f;
		if (!a) {
			return
		}
		d = a.next;
	...
	translateHeaderTransform : function(a) {
		this.header.renderElement.dom.style.webkitTransform = (a === null)
				? null
				: "translate3d(0px, -" + a + "px, 0px)"
	},
	translateHeaderCssPosition : function(a) {
		this.header.renderElement.dom.style.top = (a === null) ? null : "-"
				+ Math.round(a) + "px"
	},
	setActiveGroup : function(b) {
		var a = this, c = a.header;
		if (c) {
			if (b && b.header) {
	...
	onIndex : function(o, c) {
		var r = this, s = c.toLowerCase(), b = r.getStore(), q = b.getGroups(), f = q.length, h = r
				.getScrollable(), n, e, m, g, k, p;
		if (h) {
		...
		if (h && g) {
			...
			n.stopAnimation();
			var l = n.getContainerSize().y, j = n.getSize().y, d = j - l, a = (p.offsetTop > d)
					? d
					: p.offsetTop;
			n.scrollTo(0, a)
		}
	...
	handleItemDisclosure : function(f) {
		var d = this, c = f.getTarget().parentNode, b = d.container
				.getViewItems().indexOf(c), a = d.getStore().getAt(b);
	...
	findGroupHeaderIndices : function() {
		...
		var h = this, k = h.getStore();
		if (!k) {
			return []
		}
		var b = h.container, d = k.getGroups(), m = d.length, g = b
				.getViewItems(), c = [], l = b.footerClsShortCache, e, a, f, n, j;
		b.doRemoveHeaders();
		b.doRemoveFooterCls();
		if (g.length) {
			for (e = 0; e < m; e++) {
...
Ext.define("Ext.dataview.NestedList", {
	...
	extend : "Ext.Container",
	xtype : "nestedlist",
	requires : ["Ext.List", "Ext.Toolbar", "Ext.Button", "Ext.XTemplate",
			"Ext.data.StoreManager", "Ext.data.NodeStore", "Ext.data.TreeStore"],
	config : {
	...
	onBeforeSelect : function() {
		this.fireEvent.apply(this, [].concat("beforeselect", this,
						Array.prototype.slice.call(arguments)))
	},
	onContainerTap : function() {
		this.fireEvent.apply(this, [].concat("containertap", this,
						Array.prototype.slice.call(arguments)))
	},
	onSelectionChange : function() {
		this.fireEvent.apply(this, [].concat("selectionchange", this,
						Array.prototype.slice.call(arguments)))
	},
	onItemDoubleTap : function() {
		this.fireEvent.apply(this, [].concat("itemdoubletap", this,
						Array.prototype.slice.call(arguments)))
	},
	onStoreBeforeLoad : function() {
		...
		this.fireEvent.apply(this, [].concat("beforeload", this,
						Array.prototype.slice.call(arguments)))
	},
	onStoreLoad : function(c, b, d, a) {
		this.setMasked(false);
		this.fireEvent.apply(this, [].concat("load", this,
						Array.prototype.slice.call(arguments)));
	...
	onBackTap : function() {
		var d = this, c = d.getLastNode(), e = d.getDetailCard(), a = e
				&& d.getActiveItem() == e, b = d.getLastActiveList();
	...
	goToNode : function(a) {
		...
		var i = this, d = i.getActiveItem(), k = i.getDetailCard(), j = k
				&& i.getActiveItem() == k, h = i.goToNodeReverseAnimation(a), c = i.firstList, e = i.secondList, f = i
				.getLayout(), b = (f) ? f.getAnimation() : null, g;
		if (a.isLeaf()) {
		...
		if (j) {
			...
			i.setActiveItem(i.getLastActiveList())
		} else {
			if (c && e) {
				d = i.getActiveItem();
	...
	goToLeaf : function(f) {
		...
		var e = this, c = e.getDetailCard(f), b = e.getDetailContainer(), a = b == this, d = e
				.getLayout(), g = (d) ? d.getAnimation() : false;
		if (c) {
	...
	syncToolbar : function(g) {
		var e = this, f = e.getDetailCard(), d = e.getLastNode(), b = g
				|| (f && (e.getActiveItem() == f)), a = (b) ? d : d.parentNode, c = e
				.getBackButton();
		if (c) {
			c[a ? "show" : "hide"]();
...
Ext.define("Ext.form.Panel", {
	...
	doBeforeSubmit : function(f, h, b) {
		var e = f.element.dom || {};
		if (f.getStandardSubmit()) {
			if (b.url && Ext.isEmpty(e.action)) {
				e.action = b.url
			}
			var a = this.query("spinnerfield"), d = a.length, c, g;
			for (c = 0; c < d; c++) {
				g = a[c];
				if (!g.getDisabled()) {
					g.getComponent().setDisabled(false)
				}
			}
			e.method = (b.method || e.method).toLowerCase();
			e.submit()
		} else {
			if (b.waitMsg) {
				f.setMasked(b.waitMsg)
			}
			return Ext.Ajax.request({
				url : b.url,
				method : b.method,
				rawData : Ext.urlEncode(Ext.apply(Ext.apply({}, f
										.getBaseParams()
										|| {}), b.params || {}, h)),
				...
				scope : f,
				callback : function(i, n, j) {
					var l = this, m = j.responseText, k;
					l.setMasked(false);
					k = function() {
						if (Ext.isFunction(b.failure)) {
							b.failure.call(b.scope || l, l, j, m)
						}
						l.fireEvent("exception", l, j)
					};
					if (n) {
						j = Ext.decode(m);
						n = !!j.success;
						if (n) {
							if (Ext.isFunction(b.success)) {
								b.success.call(b.scope || l, l, j, m)
							}
							l.fireEvent("submit", l, j)
						} else {
							k()
						}
					} else {
	...
	setValues : function(b) {
		var a = this.getFields(), c, j, h, e, d, g;
		b = b || {};
		for (c in b) {
			if (b.hasOwnProperty(c)) {
				j = a[c];
	...
	getValues : function(g) {
		var f = this.getFields(), l = {}, e = Ext.isArray, j, k, b, c, a, h, d;
		b = function(m, i) {
			if (m.isCheckbox) {
				k = m.getSubmitValue()
			} else {
...
Ext.define("Ext.navigation.Bar", {
	...
	updateView : function(h) {
		var d = this, b = d.getBackButton(), f, a, e, c, g;
		d.getItems();
		if (h) {
			f = h.getInnerItems();
			for (a = 0; a < f.length; a++) {
				c = f[a];
				g = (c.getTitle) ? c.getTitle() : c.config.title;
				d.backButtonStack.push(g || "&nbsp;")
			}
			d.setTitle(d.getTitleText());
			e = d.getBackButtonText();
			if (e) {
				b.setText(e);
				b.show()
			}
	...
	doChangeView : function(k, c, g) {
		var r = this, o = r.leftBox, e = o.element, f = r.titleComponent, m = f.element, n = r
				.getBackButton(), l = r.getTitleText(), h = r
				.getBackButtonText(), q = r.getAnimation()
				&& k.getLayout().getAnimation(), p = q && q.isAnimation
				&& k.isPainted(), d, i, a, j, b;
		if (p) {
	...
	measureView : function(e, u, k) {
		var w = this, j = w.element, v = w.leftBox.element, p = w.titleComponent.element, l = Math
				.min(j.getWidth() / 3, 200), q = v.getWidth(), c = j.getX(), m = j
				.getWidth(), n = p.getX(), d = p.getLeft(), s = p.getWidth(), r = e.x, t = e.width, a = e.left, h = Ext.os.is.Android2
				&& !this.getAndroid2Transforms(), i, b, f, x, o, g;
		g = c - r - t;
		if (k) {
			i = g;
			b = Math.min(n - t, l)
	...
	animate : function(b, a, e) {
		...
		a = Ext.apply(a, {
					element : b,
					easing : "ease-in-out",
					duration : c.getAnimation().duration
				});
	...
	getBackButtonText : function() {
		var b = this.backButtonStack[this.backButtonStack.length - 2], a = this
...
Ext.define("Ext.navigation.View", {
			...
			beforePop : function(e) {
				var d = this, f = this.getInnerItems(), c = f.length, b, a;
				if (!Ext.isNumber(e) || e < 1) {
					e = 1
				}
				e = Math.min(e, c - 1);
				if (e) {
					d.getNavigationBar().beforePop(e);
					b = f.splice(-e, e - 1);
			...
			doPop : function() {
				var a = this, b = this.getInnerItems();
				a.remove(b[b.length - 1]);
			...
			applyNavigationBar : function(a) {
				...
				a.view = this;
				a.useTitleForBackButtonText = this
						.getUseTitleForBackButtonText();
				return Ext.factory(a, Ext.navigation.Bar, this
								.getNavigationBar())
			},
			updateNavigationBar : function(a, c) {
				if (c) {
					this.remove(c, true)
				}
				if (a) {
					var b = this.getLayout(), d = (b && b.isLayout) ? b
							.getAnimation() : false;
					if (d && d.isAnimation) {
						a.setAnimation(d.config)
					}
					this.add(a)
				}
			},
			...
			doResetActiveItem : function(d) {
				var a = this, c = a.getInnerItems(), b = a.getLayout()
						.getAnimation();
				if (d > 0) {
					if (b && b.isAnimation) {
						b.setReverse(true)
...
Ext.define("Ext.picker.Slot", {
	...
	requires : ["Ext.XTemplate", "Ext.data.Store", "Ext.Component",
	...
	updateDisplayField : function(a) {
		this.setItemTpl('<div class="' + Ext.baseCSSPrefix
				+ 'picker-item {cls} <tpl if="extra">' + Ext.baseCSSPrefix
				+ 'picker-invalid</tpl>">{' + a + "}</div>")
	},
	...
	setupBar : function() {
		...
		var a = this.element, e = this.innerElement, f = this.getPicker(), g = f.bar, k = this
				.getValue(), b = this.getShowTitle(), j = this.getTitle(), h = this
				.getScrollable(), d = h.getScroller(), c = 0, l, i;
		l = g.getHeight();
		if (b && j) {
	...
	scrollToItem : function(e, d) {
		var h = e.getY(), c = e.parent(), f = c.getY(), b = this
				.getScrollable(), a = b.getScroller(), g;
		g = h - f;
		a.scrollTo(0, g, d)
	},
	onScrollEnd : function(b, a, g) {
		var f = this, d = Math.round(g / f.picker.bar.getHeight()), c = f
				.getViewItems(), e = c[d];
		if (e) {
			f.selectedIndex = d;
			f.selectedNode = e;
	...
	setValue : function(f) {
		...
		var b = this.getStore(), a = this.getViewItems(), d = this
				.getValueField(), c, e;
		c = b.find(d, f);
		if (c != -1) {
			e = Ext.get(a[c]);
...
Ext.define("Ext.picker.Picker", {
			...
			updateUseTitles : function(d) {
				var f = this.getInnerItems(), e = f.length, a = Ext.baseCSSPrefix
						+ "use-titles", c, b;
				if (d) {
					this.addCls(a)
				} else {
...
Ext.define("Ext.field.Select", {
	...
	requires : ["Ext.Panel", "Ext.picker.Picker", "Ext.data.Store",
			"Ext.data.StoreManager", "Ext.dataview.List"],
	config : {
		ui : "select",
		valueField : "value",
		displayField : "text",
	...
	updateValue : function(b, a) {
		...
		this.record = b;
		this
				.callParent([(b && b.isModel)
						? b.get(this.getDisplayField())
						: ""])
	},
	getValue : function() {
		var a = this.record;
	...
	showPicker : function() {
		...
		if (this.getUsePicker()) {
			var e = this.getPhonePicker(), d = this.getName(), h = {};
			...
			var f = this.getTabletPicker(), g = f.down("list"), b = g
					.getStore(), c = b.find(this.getValueField(), this
							.getValue(), null, null, null, true), a = b
					.getAt((c == -1) ? 0 : c);
			if (!f.getParent()) {
				Ext.Viewport.add(f)
			}
	...
	onPickerChange : function(d, f) {
		var e = this, g = f[e.getName()], b = e.getStore(), c = b.find(e
						.getValueField(), g, null, null, null, true), a = b
				.getAt(c);
		e.setValue(a)
	},
	onChange : function(f, h, e) {
		var g = this, b = g.getStore(), d = (b) ? b
				.find(g.getDisplayField(), e) : -1, c = g.getValueField(), a = (b)
				? b.getAt(d)
				: null, e = (a) ? a.get(c) : null;
	...
	reset : function() {
		var b = this.getStore(), a = (this.originalValue)
				? this.originalValue
				: b.getAt(0);
		if (b && a) {
			this.setValue(a)
...
Ext.define("Ext.picker.Date", {
	...
	updateMonthText : function(a, b) {
		...
		if (this.initialized) {
			for (c = 0; c < e; c++) {
				d = f[c];
				if ((typeof d.title == "string" && d.title == b)
						|| (d.title.html == b)) {
					d.setTitle(a)
				}
			}
	...
	updateDayText : function(a, c) {
		...
		if (this.initialized) {
			for (b = 0; b < e; b++) {
				d = f[b];
				if ((typeof d.title == "string" && d.title == c)
						|| (d.title.html == c)) {
					d.setTitle(a)
				}
			}
	...
	createSlots : function() {
		var k = this, c = this.getSlotOrder(), m = k.getYearFrom(), f = k
				.getYearTo(), g = [], l = [], b = [], j, e, d, a;
		if (m > f) {
			e = m;
			m = f;
			f = e
	...
	onSlotPick : function() {
		var g = this.getValue(true), f = this.getDaySlot(), e = g.getFullYear(), d = g
				.getMonth(), j = [], a, b;
		if (!g || !Ext.isDate(g) || !f) {
			return
		}
...
Ext.define("Ext.field.DatePicker", {
			...
			updateValue : function(b) {
				...
				if (b !== null) {
					this.getComponent().setValue(Ext.Date.format(b, this
									.getDateFormat()
			...
			updateDateFormat : function(c, a) {
				var b = this.getValue();
				if (c != a && Ext.isDate(b)) {
					this.getComponent().setValue(Ext.Date.format(b, c
									|| Ext.util.Format.defaultDateFormat))
				}
			},
			getValue : function() {
			...
			getFormattedValue : function(b) {
				var a = this.getValue();
				return (Ext.isDate(a)) ? Ext.Date.format(a, b
								|| this.getDateFormat()
								|| Ext.util.Format.defaultDateFormat) : a
			},
...
Ext.define("Ext.slider.Slider", {
	...
	refreshOffsetValueRatio : function() {
		var b = this.getMaxValue() - this.getMinValue(), a = this.elementWidth
				- this.thumbWidth;
	...
	onThumbDrag : function(c, g, a) {
		var d = this.getThumbIndex(c), f = this.offsetValueRatio, b = this
				.constrainValue(a / f);
		g.stopPropagation();
	...
	setIndexValue : function(d, g, f) {
		var c = this.getThumb(d), b = this.getValue(), e = this.offsetValueRatio, a = c
				.getDraggable();
	...
	refreshThumbConstraints : function(d) {
		var b = this.getAllowThumbsOverlapping(), a = d.getDraggable()
				.getOffset().x, c = this.getThumbs(), e = this.getThumbIndex(d), g = c[e
				- 1], h = c[e + 1], f = this.thumbWidth;
		if (g) {
	...
	onTap : function(j) {
		...
		var n = j.touch.point.x, h = this.element, c = h.getX(), d = n - c
				- (this.thumbWidth / 2), o = this.constrainValue(d
				/ this.offsetValueRatio), r = this.getValue(), q = Infinity, m = r.length, g, f, l, p, b, a;
		if (m === 1) {
			p = 0
		} else {
			for (g = 0; g < m; g++) {
	...
	updateValue : function(e, b) {
		var a = this.getThumbs(), d = e.length, c;
		this.setThumbsCount(d);
		for (c = 0; c < d; c++) {
			a[c].getDraggable().setExtraConstraint(null).setOffset(e[c]
					* this.offsetValueRatio)
		}
		for (c = 0; c < d; c++) {
	...
	constrainValue : function(e) {
		var b = this, d = b.getMinValue(), f = b.getMaxValue(), a = b
				.getIncrement(), c;
		e = parseFloat(e);
		if (isNaN(e)) {
			e = d
...
Ext.define("Ext.field.Slider", {
			...
			onSliderChange : function() {
				this.fireEvent.apply(this, [].concat("change", this,
								Array.prototype.slice.call(arguments)))
			},
			onSliderDragStart : function() {
				this.fireEvent.apply(this, [].concat("dragstart", this,
								Array.prototype.slice.call(arguments)))
			},
			onSliderDrag : function() {
				this.fireEvent.apply(this, [].concat("drag", this,
								Array.prototype.slice.call(arguments)))
			},
			onSliderDragEnd : function() {
				this.fireEvent.apply(this, [].concat("dragend", this,
								Array.prototype.slice.call(arguments)))
			},
			setValues : function(a) {
				this.setValue(a)
			...
			reset : function() {
				var b = this.config, a = (this.config.hasOwnProperty("values"))
						? b.values
						: b.value;
				this.setValue(a)
			},
...
Ext.define("Ext.slider.Toggle", {
			...
			onChange : function(d, a, f, c) {
				var g = f > 0, b = d.getMaxValueCls(), e = d.getMinValueCls();
			...
			onTap : function() {
				...
				var b = this.getValue(), c = (b == 1) ? 0 : 1, a = this
						.getThumb(0);
				this.setIndexValue(0, c, this.getAnimation());
				this.refreshThumbConstraints(a);
				this.fireEvent("change", this, a, c, b)
			}
		});
...
Ext.define("Ext.tab.Tab", {
			...
			hideIconElement : function() {
				this.iconElement.dom.style.setProperty("visibility", "hidden",
						"!important")
			},
			showIconElement : function() {
				this.iconElement.dom.style.setProperty("visibility", "visible",
						"!important")
			},
...
Ext.define("Ext.tab.Bar", {
			...
			doSetDocked : function(a) {
				var c = this.getLayout(), b = a == "bottom" ? "center" : "left";
				if (c.isLayout) {
					c.setPack(b)
				} else {
...
Ext.define("Ext.tab.Panel", {
	...
	doSetActiveItem : function(d, j) {
		if (d) {
			var f = this.getInnerItems(), g = f.indexOf(j), i = f.indexOf(d), e = g > i, c = this
					.getLayout().getAnimation(), b = this.getTabBar(), h = b
					.parseActiveTab(g), a = b.parseActiveTab(i);
			if (c && c.setReverse) {
				c.setReverse(e)
			}
			this.callParent(arguments);
	...
	onItemAdd : function(e) {
		...
		var c = k.getTabBar(), o = e.getInitialConfig(), d = o.tab || {}, g = (e.getTitle)
				? e.getTitle()
				: o.title, i = (e.getIconCls) ? e.getIconCls() : o.iconCls, j = (e.getHidden)
				? e.getHidden()
				: o.hidden, n = (e.getDisabled) ? e.getDisabled() : o.disabled, p = (e.getBadgeText)
				? e.getBadgeText()
				: o.badgeText, b = k.getInnerItems(), h = b.indexOf(e), l = c
				.getItems(), a = c.getActiveTab(), m = (l.length >= b.length)
				&& l.getAt(h), f;
		if (g && !d.title) {
			d.title = g
		}
...
Ext.define("Ext.viewport.Default", {
	...
	constructor : function(a) {
		...
		if (this.supportsOrientation()) {
			this.addWindowListener("orientationchange", b(
		...
		this.getEventDispatcher().addListener("component", "*", "fullscreen",
				"onItemFullscreenChange", this);
		return this
	},
	onDomReady : function() {
		this.isReady = true;
	...
	render : function() {
		if (!this.rendered) {
			var a = Ext.getBody(), b = Ext.baseCSSPrefix, h = [], d = Ext.os, g = d.name
					.toLowerCase(), f = Ext.browser.name.toLowerCase(), e = d.version
					.getMajor(), c = this.getOrientation();
			this.renderTo(a);
	...
	applyAutoMaximize : function(a) {
		...
		if (a) {
			...
			this.on("orientationchange", "doAutoMaximizeOnOrientationChange",
					this)
		} else {
			this.un("ready", "doAutoMaximizeOnReady", this);
			this.un("orientationchange", "doAutoMaximizeOnOrientationChange",
					this)
		}
		return a
	},
	applyPreventPanning : function(a) {
		if (a) {
			...
			this
					.removeWindowListener("touchmove", this.doPreventPanning,
							false)
		}
		return a
	},
	...
	onResize : function() {
		var c = this.windowWidth, f = this.windowHeight, e = this
				.getWindowWidth(), a = this.getWindowHeight(), d = this
				.getOrientation(), b = this.determineOrientation();
		if ((c !== e || f !== a) && d !== b) {
	...
	fireOrientationChangeEvent : function(b, c) {
		...
		this.orientation = b;
		this.updateSize();
		this.fireEvent("orientationchange", this, b, this.windowWidth,
				this.windowHeight)
	},
...
Ext.define("Ext.viewport.Android", {
	...
	getDummyInput : function() {
		var a = this.dummyInput, c = this.focusedElement, b = Ext.fly(c)
				.getPageBox();
		if (!a) {
	...
	getActualWindowOuterHeight : function() {
		return Math
				.round(this.getWindowOuterHeight() / window.devicePixelRatio)
	},
	maximize : function() {
	...
	var a = Ext.os.version, b = Ext.browser.userAgent, c = /(htc|desire|incredible|ADR6300)/i
			.test(b)
			&& a.lt("2.3");
	if (c) {
		this.override({
					constructor : function(d) {
						...
						this.watchDogTick = Ext.Function.bind(
								this.watchDogTick, this);
					...
					doPreventPanning : function() {
						var e = Ext.Date.now(), f = this.watchDogLastTick, d = e
								- f;
						if (d >= 2000) {
							return
						}
	...
	if (a.match("2")) {
		this.override({
			onReady : function() {
				this.addWindowListener("resize", Ext.Function.bind(
			...
			onWindowResize : function() {
				var e = this.windowWidth, g = this.windowHeight, f = this
						.getWindowWidth(), d = this.getWindowHeight();
				if (this.getAutoMaximize()
						...
						&& e === f
						&& d < g
						&& ((d >= g - this.addressBarHeight) || !this.focusedElement)) {
					this.scrollToTop()
				}
			},
			fixSize : function() {
				var d = this.getOrientation(), f = window.outerHeight, g = window.outerWidth, e;
				if (d === "landscape" && (f < g) || d === "portrait"
						&& (f >= g)) {
...
Ext.define("Ext.viewport.Ios", {
			...
			constructor : function() {
				...
				if (this.getAutoMaximize() && !this.isFullscreen()) {
					this.addWindowListener("touchstart", Ext.Function.bind(
			...
			maximize : function() {
				...
				var c = this.stretchHeights, b = this.orientation, d = this
						.getWindowHeight(), a = c[b];
				if (window.scrollY > 0) {
					this.scrollToTop();
					if (!a) {
			...
			getScreenHeight : function() {
				return window.screen[this.orientation === this.PORTRAIT
						? "height"
						: "width"]
			},
...
Ext.define("Ext.event.recognizer.Swipe", {
	...
	onTouchMove : function(f) {
		var h = f.changedTouches[0], b = h.pageX, g = h.pageY, c = Math.abs(b
				- this.startX), a = Math.abs(g - this.startY), d = f.time;
	...
	onTouchEnd : function(i) {
		...
		var h = i.changedTouches[0], l = h.pageX, j = h.pageY, g = l
				- this.startX, f = j - this.startY, c = Math.abs(g), b = Math
				.abs(f), m = this.getMinDistance(), d = i.time - this.startTime, k, a;
...
Ext.define("Ext.event.recognizer.HorizontalSwipe", {
	...
	onTouchMove : function(f) {
		var h = f.changedTouches[0], g = h.pageY, a = Math.abs(g - this.startY), d = f.time, c = this
				.getMaxDuration(), b = this.getMaxOffset();
	...
	onTouchEnd : function(f) {
		if (this.onTouchMove(f) !== false) {
			var i = f.changedTouches[0], a = i.pageX, b = a - this.startX, h = Math
					.abs(b), d = f.time - this.startTime, g = this
					.getMinDistance(), c;
			if (h < g) {
				return this.fail(this.self.DISTANCE_NOT_ENOUGH)
			}
